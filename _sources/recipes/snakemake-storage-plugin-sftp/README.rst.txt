...
.. conda:package:: snakemake-storage-plugin-sftp
   ...
   :depends on pysftp: ``>=0.2.9,<0.3.0``
   :depends on python: ``>=3.11.0,<4.0.0``
   :depends on snakemake-interface-common: ``>=1.14.3,<2.0.0``
...
to install globally, run::
    ...
    pixi global install snakemake-storage-plugin-sftp

to add into an existing workspace instead, run::

    pixi add snakemake-storage-plugin-sftp
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install snakemake-storage-plugin-sftp

Alternatively, to install into a new environment, run::

    conda create -n envname snakemake-storage-plugin-sftp
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_snakemake-storage-plugin-sftp"></div>
   <div style="width: 100%" id="platform_plot_snakemake-storage-plugin-sftp"></div>
   <div style="width: 100%" id="cdf_plot_snakemake-storage-plugin-sftp"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for snakemake-storage-plugin-sftp
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-storage-plugin-sftp/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_snakemake-storage-plugin-sftp', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for snakemake-storage-plugin-sftp
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-storage-plugin-sftp/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_snakemake-storage-plugin-sftp', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for snakemake-storage-plugin-sftp
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-storage-plugin-sftp/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_snakemake-storage-plugin-sftp', spec);
            } catch (err) {
