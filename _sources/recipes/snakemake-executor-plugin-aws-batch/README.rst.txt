...
.. conda:package:: snakemake-executor-plugin-aws-batch
   ...
   :depends on boto3: ``>=1.36.5``
   :depends on python: 
   :depends on snakemake-interface-common: ``>=1.17.4``
   :depends on snakemake-interface-executor-plugins: ``>=9.3.2``
   :depends on snakemake-storage-plugin-s3: ``>=0.3.1``
...
to install globally, run::
    ...
    pixi global install snakemake-executor-plugin-aws-batch

to add into an existing workspace instead, run::

    pixi add snakemake-executor-plugin-aws-batch
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install snakemake-executor-plugin-aws-batch

Alternatively, to install into a new environment, run::

    conda create -n envname snakemake-executor-plugin-aws-batch
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_snakemake-executor-plugin-aws-batch"></div>
   <div style="width: 100%" id="platform_plot_snakemake-executor-plugin-aws-batch"></div>
   <div style="width: 100%" id="cdf_plot_snakemake-executor-plugin-aws-batch"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for snakemake-executor-plugin-aws-batch
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-executor-plugin-aws-batch/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_snakemake-executor-plugin-aws-batch', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for snakemake-executor-plugin-aws-batch
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-executor-plugin-aws-batch/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_snakemake-executor-plugin-aws-batch', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for snakemake-executor-plugin-aws-batch
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-executor-plugin-aws-batch/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_snakemake-executor-plugin-aws-batch', spec);
            } catch (err) {
