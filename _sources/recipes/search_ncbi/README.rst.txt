...
to install globally, run::
    ...
    pixi global install search_ncbi

to add into an existing workspace instead, run::

    pixi add search_ncbi
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install search_ncbi

Alternatively, to install into a new environment, run::

    conda create -n envname search_ncbi
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_search_ncbi"></div>
   <div style="width: 100%" id="platform_plot_search_ncbi"></div>
   <div style="width: 100%" id="cdf_plot_search_ncbi"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for search_ncbi
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/search_ncbi/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_search_ncbi', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for search_ncbi
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/search_ncbi/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_search_ncbi', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for search_ncbi
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/search_ncbi/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_search_ncbi', spec);
            } catch (err) {
