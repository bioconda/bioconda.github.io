...
.. conda:recipe:: somalier
   ...
   :documentation: https://github.com/brentp/somalier/blob/v0.3.2/README.md
...
.. conda:package:: somalier
   ...
   :versions:
      ...
      ``0.3.2-0``,  ``0.3.1-0``,  ``0.3.0-0``,  ``0.2.19-0``,  ``0.2.18-0``,  ``0.2.17-0``,  ``0.2.15-1``,  ``0.2.15-0``
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["0.3.2","0.3.1","0.3.0","0.2.19","0.2.18"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_somalier"></div>
   <div style="width: 100%" id="platform_plot_somalier"></div>
   <div style="width: 100%" id="cdf_plot_somalier"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/somalier/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_somalier', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for somalier
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/somalier/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_somalier', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for somalier
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/somalier/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_somalier', spec);
            } catch (err) {
