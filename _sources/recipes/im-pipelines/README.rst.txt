...
.. conda:package:: im-pipelines
   ...
   :depends on im-pipelines-utils: 
   :depends on im-pipelines-utils-rdkit: 
   :depends on matplotlib-base: 
   :depends on molvs: 
   :depends on python: 
   :depends on scikit-learn: 
   :depends on standardiser: 
...
to install globally, run::
    ...
    pixi global install im-pipelines

to add into an existing workspace instead, run::

    pixi add im-pipelines
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname im-pipelines
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_im-pipelines"></div>
   <div style="width: 100%" id="platform_plot_im-pipelines"></div>
   <div style="width: 100%" id="cdf_plot_im-pipelines"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for im-pipelines
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/im-pipelines/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_im-pipelines', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for im-pipelines
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/im-pipelines/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_im-pipelines', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for im-pipelines
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/im-pipelines/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_im-pipelines', spec);
            } catch (err) {
