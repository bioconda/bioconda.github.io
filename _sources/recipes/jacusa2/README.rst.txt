...
.. conda:package:: jacusa2
   ...
   :versions:
      ...
      ``2.1.17-0``,  ``2.1.16-0``,  ``2.1.15-0``,  ``2.0.4-0``
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install jacusa2

Alternatively, to install into a new environment, run::

    conda create -n envname jacusa2
...
.. raw:: html
   ...
   <script>
      var package = "jacusa2";
      var versions = ["2.1.17","2.1.16","2.1.15","2.0.4"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_jacusa2"></div>
   <div style="width: 100%" id="platform_plot_jacusa2"></div>
   <div style="width: 100%" id="cdf_plot_jacusa2"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/jacusa2/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_jacusa2', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for jacusa2
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/jacusa2/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_jacusa2', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for jacusa2
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/jacusa2/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_jacusa2', spec);
            } catch (err) {
