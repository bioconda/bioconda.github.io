...
to install globally, run::
    ...
    pixi global install perl-test-unit-lite

to add into an existing workspace instead, run::

    pixi add perl-test-unit-lite
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname perl-test-unit-lite
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_perl-test-unit-lite"></div>
   <div style="width: 100%" id="platform_plot_perl-test-unit-lite"></div>
   <div style="width: 100%" id="cdf_plot_perl-test-unit-lite"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for perl-test-unit-lite
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/perl-test-unit-lite/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_perl-test-unit-lite', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for perl-test-unit-lite
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/perl-test-unit-lite/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_perl-test-unit-lite', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for perl-test-unit-lite
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/perl-test-unit-lite/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_perl-test-unit-lite', spec);
            } catch (err) {
