...
to install globally, run::
    ...
    pixi global install ffindex-dbkit

to add into an existing workspace instead, run::

    pixi add ffindex-dbkit
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install ffindex-dbkit

Alternatively, to install into a new environment, run::

    conda create -n envname ffindex-dbkit
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ffindex-dbkit"></div>
   <div style="width: 100%" id="platform_plot_ffindex-dbkit"></div>
   <div style="width: 100%" id="cdf_plot_ffindex-dbkit"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ffindex-dbkit
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ffindex-dbkit/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ffindex-dbkit', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ffindex-dbkit
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ffindex-dbkit/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ffindex-dbkit', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ffindex-dbkit
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ffindex-dbkit/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ffindex-dbkit', spec);
            } catch (err) {
