...
.. conda:package:: alv
   ...
   :versions:
      ...
      ``1.8.3-0``,  ``1.8.2-0``,  ``1.8.1-0``,  ``1.7.2-0``,  ``1.7.1-0``,  ``1.7.0-0``,  ``1.6.0-0``,  ``1.5.1-0``
...
.. raw:: html
   ...
   <script>
      var package = "alv";
      var versions = ["1.8.3","1.8.2","1.8.1","1.7.2","1.7.1"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_alv"></div>
   <div style="width: 100%" id="platform_plot_alv"></div>
   <div style="width: 100%" id="cdf_plot_alv"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/alv/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_alv', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for alv
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/alv/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_alv', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for alv
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/alv/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_alv', spec);
            } catch (err) {
