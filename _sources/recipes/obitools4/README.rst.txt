...
.. conda:package:: obitools4
   ...
   :versions:
      ...
      ``4.5.0-0``,  ``4.4.46-0``,  ``4.4.45-0``,  ``4.4.42-0``,  ``4.4.41-0``,  ``4.4.0-0``
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install obitools4

Alternatively, to install into a new environment, run::

    conda create -n envname obitools4
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["4.5.0","4.4.46","4.4.45","4.4.42","4.4.41"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_obitools4"></div>
   <div style="width: 100%" id="platform_plot_obitools4"></div>
   <div style="width: 100%" id="cdf_plot_obitools4"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/obitools4/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_obitools4', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for obitools4
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/obitools4/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_obitools4', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for obitools4
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/obitools4/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_obitools4', spec);
            } catch (err) {
