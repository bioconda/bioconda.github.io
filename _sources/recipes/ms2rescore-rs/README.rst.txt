...
.. conda:package:: ms2rescore-rs
   ...
   :versions:
      ...
      ``0.5.0-0``,  ``0.4.3-1``,  ``0.4.3-0``,  ``0.4.2-0``,  ``0.4.1-0``,  ``0.2.0-2``,  ``0.2.0-1``,  ``0.2.0-0``,  ``0.1.0-0``

      

   
   :depends on __glibc: ``>=2.17,<3.0.a0``
   :depends on dotnet-runtime: ``6.0.*``
...
to install globally, run::
    ...
    pixi global install ms2rescore-rs

to add into an existing workspace instead, run::

    pixi add ms2rescore-rs
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install ms2rescore-rs

Alternatively, to install into a new environment, run::

    conda create -n envname ms2rescore-rs
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["0.5.0","0.4.3","0.4.3","0.4.2","0.4.1"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ms2rescore-rs"></div>
   <div style="width: 100%" id="platform_plot_ms2rescore-rs"></div>
   <div style="width: 100%" id="cdf_plot_ms2rescore-rs"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ms2rescore-rs
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ms2rescore-rs/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ms2rescore-rs', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ms2rescore-rs
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ms2rescore-rs/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ms2rescore-rs', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ms2rescore-rs
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ms2rescore-rs/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ms2rescore-rs', spec);
            } catch (err) {
