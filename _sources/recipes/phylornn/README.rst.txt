...
.. conda:package:: phylornn
   ...
   :depends on dendropy: ``>=4.5.2,<4.6``
   :depends on ete3: 
   :depends on keras: ``>=2.9.0,<2.10``
   :depends on matplotlib-base: ``>=3.5.2,<3.6``
   :depends on numpy: ``>=1.23.1,<1.24``
   :depends on pandas: ``>=1.4.3,<1.5``
   :depends on phyml: ``>=3.3.0,<3.4``
   :depends on python: ``>=3.8``
   :depends on scipy: ``>=1.8.1,<1.9``
   :depends on seaborn-base: ``>=0.11.2,<0.12``
   :depends on seq-gen: ``>=1.3.0,<1.4``
   :depends on tensorflow: ``>=2.9.1,<3``
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install phylornn

Alternatively, to install into a new environment, run::

    conda create -n envname phylornn
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_phylornn"></div>
   <div style="width: 100%" id="platform_plot_phylornn"></div>
   <div style="width: 100%" id="cdf_plot_phylornn"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/phylornn/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_phylornn', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for phylornn
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/phylornn/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_phylornn', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for phylornn
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/phylornn/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_phylornn', spec);
            } catch (err) {
