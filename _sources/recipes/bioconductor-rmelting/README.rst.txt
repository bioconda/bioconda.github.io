...
.. conda:recipe:: bioconductor-rmelting
   ...
   :homepage: https://bioconductor.org/packages/3.22/bioc/html/rmelting.html
   :license: GPL-2 | GPL-3
...
.. conda:package:: bioconductor-rmelting
   ...
   :versions:
      ...
      ``1.26.0-0``,  ``1.22.0-0``,  ``1.18.0-0``,  ``1.16.0-0``,  ``1.14.0-0``,  ``1.10.0-0``,  ``1.8.0-0``,  ``1.6.0-1``,  ``1.6.0-0``,  ``1.2.0-0``,  ``1.0.0-1``
...
to install globally, run::
    ...
    pixi global install bioconductor-rmelting

to add into an existing workspace instead, run::

    pixi add bioconductor-rmelting
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-rmelting
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-rmelting"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-rmelting"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-rmelting"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-rmelting
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-rmelting/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-rmelting', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-rmelting
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-rmelting/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-rmelting', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-rmelting
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-rmelting/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-rmelting', spec);
            } catch (err) {
