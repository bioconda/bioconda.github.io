...
.. conda:package:: pbhoover
   ...
   :versions:
      ...
      ``1.1.0-1``,  ``1.1.0-0``,  ``1.0.7-5``,  ``1.0.7-4``,  ``1.0.7-3``,  ``1.0.7-2``,  ``1.0.7-0``,  ``1.0.6-0``

      

   
   :depends on bcftools: ``<=1.3.1``
   :depends on htslib: ``<=1.3.2``
   :depends on pbcore: ``<=1.2.7``
   :depends on pbh5tools: ``<=0.8.0``
   :depends on python: ``<3``
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname pbhoover
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["1.1.0","1.1.0","1.0.7","1.0.7","1.0.7"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_pbhoover"></div>
   <div style="width: 100%" id="platform_plot_pbhoover"></div>
   <div style="width: 100%" id="cdf_plot_pbhoover"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/pbhoover/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_pbhoover', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for pbhoover
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/pbhoover/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_pbhoover', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for pbhoover
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/pbhoover/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_pbhoover', spec);
            } catch (err) {
