...
.. conda:package:: super_distance
   ...
   :versions:
      ...
      ``1.1.0-6``,  ``1.1.0-5``,  ``1.1.0-4``,  ``1.1.0-3``,  ``1.1.0-2``,  ``1.1.0-1``,  ``1.1.0-0``,  ``0.1-0``
...
to install globally, run::
    ...
    pixi global install super_distance

to add into an existing workspace instead, run::

    pixi add super_distance
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname super_distance
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_super_distance"></div>
   <div style="width: 100%" id="platform_plot_super_distance"></div>
   <div style="width: 100%" id="cdf_plot_super_distance"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for super_distance
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/super_distance/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_super_distance', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for super_distance
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/super_distance/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_super_distance', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for super_distance
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/super_distance/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_super_distance', spec);
            } catch (err) {
