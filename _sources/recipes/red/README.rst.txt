...
.. conda:package:: red
   ...
   :versions:
      ...
      ``2018.09.10-3``,  ``2018.09.10-2``,  ``2018.09.10-1``,  ``2018.09.10-0``,  ``2015.05.22-3``,  ``2015.05.22-2``,  ``2015.05.22-1``,  ``2015.05.22-0``
...
.. raw:: html
   ...
   <script>
      var package = "red";
      var versions = ["2018.09.10","2018.09.10","2018.09.10","2018.09.10","2015.05.22"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_red"></div>
   <div style="width: 100%" id="platform_plot_red"></div>
   <div style="width: 100%" id="cdf_plot_red"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/red/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_red', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for red
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/red/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_red', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for red
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/red/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_red', spec);
            } catch (err) {
