...
.. conda:package:: cdhit-reader
   ...
   :versions:
      ...
      ``0.5.0-0``,  ``0.3.0-0``,  ``0.2.0-0``,  ``0.1.1-0``,  ``0.1.0-0``,  ``0.0.6-0``
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["0.5.0","0.3.0","0.2.0","0.1.1","0.1.0"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_cdhit-reader"></div>
   <div style="width: 100%" id="platform_plot_cdhit-reader"></div>
   <div style="width: 100%" id="cdf_plot_cdhit-reader"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for cdhit-reader
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/cdhit-reader/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_cdhit-reader', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for cdhit-reader
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/cdhit-reader/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_cdhit-reader', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for cdhit-reader
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/cdhit-reader/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_cdhit-reader', spec);
            } catch (err) {
