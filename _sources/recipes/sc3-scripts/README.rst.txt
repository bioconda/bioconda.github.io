...
.. conda:package:: sc3-scripts
   ...
   :depends on bioconductor-sc3: ``>=1.10.0,<1.11``
   :depends on bioconductor-singlecellexperiment-scripts: 
   :depends on r-base: ``>=3.5.1,<3.5.2.0a0``
   :depends on r-optparse: 
   :depends on r-workflowscriptscommon: 
   :depends on scater-scripts: 
...
to install globally, run::
    ...
    pixi global install sc3-scripts

to add into an existing workspace instead, run::

    pixi add sc3-scripts
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname sc3-scripts
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_sc3-scripts"></div>
   <div style="width: 100%" id="platform_plot_sc3-scripts"></div>
   <div style="width: 100%" id="cdf_plot_sc3-scripts"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for sc3-scripts
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/sc3-scripts/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_sc3-scripts', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for sc3-scripts
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/sc3-scripts/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_sc3-scripts', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for sc3-scripts
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/sc3-scripts/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_sc3-scripts', spec);
            } catch (err) {
