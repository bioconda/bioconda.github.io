...
to install globally, run::
    ...
    pixi global install ucsc-chainprenet

to add into an existing workspace instead, run::

    pixi add ucsc-chainprenet
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname ucsc-chainprenet
...
For e.g. docker, run::
    ...
    docker pull quay.io/biocontainers/ucsc-chainprenet:<tag>

(see `ucsc-chainprenet/tags`_ for valid values for ``<tag>``).
...
.. _`ucsc-chainprenet/tags`: https://quay.io/repository/biocontainers/ucsc-chainprenet?tab=tags


.. raw:: html

   <script>
      var package = "ucsc-chainprenet";
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ucsc-chainprenet"></div>
   <div style="width: 100%" id="platform_plot_ucsc-chainprenet"></div>
   <div style="width: 100%" id="cdf_plot_ucsc-chainprenet"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ucsc-chainprenet
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-chainprenet/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ucsc-chainprenet', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ucsc-chainprenet
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-chainprenet/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ucsc-chainprenet', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ucsc-chainprenet
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-chainprenet/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ucsc-chainprenet', spec);
            } catch (err) {
