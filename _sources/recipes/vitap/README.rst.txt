...
.. conda:recipe:: vitap
   ...
   Viral Taxonomic Assignment Pipeline \(VITAP\) is a tool designed to address the growing need for accurate and comprehensive taxonomic assignments of DNA and RNA viral sequences based on ICTV VMR and UniRef90.
...
.. conda:package:: vitap
   ...
   :versions:
      ...
      ``1.12-0``,  ``1.10-0``,  ``1.7.1-0``,  ``1.7-1``,  ``1.7-0``,  ``1.5-0``,  ``1.2-0``,  ``1.1-0``
...
.. raw:: html
   ...
   <script>
      var package = "vitap";
      var versions = ["1.12","1.10","1.7.1","1.7","1.7"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_vitap"></div>
   <div style="width: 100%" id="platform_plot_vitap"></div>
   <div style="width: 100%" id="cdf_plot_vitap"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/vitap/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_vitap', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for vitap
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/vitap/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_vitap', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for vitap
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/vitap/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_vitap', spec);
            } catch (err) {
