...
.. conda:package:: phytest
   ...
   :depends on pytest: ``>=7.1.1``
   :depends on pytest-html: ``>=3.1.1``
   :depends on pytest-sugar: ``>=0.9.4``
   :depends on pytest-xdist: ``>=3.2.0``
   :depends on python: ``>=3.8,<=3.11``
   :depends on scipy: ``>=1.8.0``
   :depends on treetime: ``>=0.8.6``
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install phytest

Alternatively, to install into a new environment, run::

    conda create -n envname phytest
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_phytest"></div>
   <div style="width: 100%" id="platform_plot_phytest"></div>
   <div style="width: 100%" id="cdf_plot_phytest"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/phytest/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_phytest', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for phytest
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/phytest/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_phytest', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for phytest
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/phytest/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_phytest', spec);
            } catch (err) {
