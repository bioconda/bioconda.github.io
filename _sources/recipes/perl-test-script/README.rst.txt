...
to install globally, run::
    ...
    pixi global install perl-test-script

to add into an existing workspace instead, run::

    pixi add perl-test-script
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname perl-test-script
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["1.31","1.30","1.29","1.25"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_perl-test-script"></div>
   <div style="width: 100%" id="platform_plot_perl-test-script"></div>
   <div style="width: 100%" id="cdf_plot_perl-test-script"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for perl-test-script
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/perl-test-script/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_perl-test-script', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for perl-test-script
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/perl-test-script/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_perl-test-script', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for perl-test-script
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/perl-test-script/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_perl-test-script', spec);
            } catch (err) {
