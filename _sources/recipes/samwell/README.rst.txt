...
.. conda:package:: samwell
   ...
   :versions:
      ...
      ``0.0.4-3``,  ``0.0.4-2``,  ``0.0.4-1``,  ``0.0.4-0``,  ``0.0.3-1``,  ``0.0.3-0``,  ``0.0.2-0``
   ...
   :depends on cython: 
   :depends on defopt: ``>=6.0``
   :depends on intervaltree: 
   :depends on libgcc: ``>=14``
   :depends on mypy_extensions: ``>=0.4.3``
   :depends on pybedlite: ``>=0.0.1``
   :depends on pysam: ``>=0.15.3``
   :depends on python: ``>=3.10,<3.11.0a0``
   :depends on python_abi: ``3.10.* *_cp310``
   :depends on typing_extensions: ``>=3.7.4``
   :depends on typing_inspect: ``>=0.3.1``
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname samwell
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_samwell"></div>
   <div style="width: 100%" id="platform_plot_samwell"></div>
   <div style="width: 100%" id="cdf_plot_samwell"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/samwell/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_samwell', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for samwell
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/samwell/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_samwell', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for samwell
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/samwell/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_samwell', spec);
            } catch (err) {
