...
.. conda:package:: perl-digest-sha1
   ...
   :versions:
      ...
      ``2.13-8``,  ``2.13-7``,  ``2.13-6``,  ``2.13-5``,  ``2.13-4``,  ``2.13-3``,  ``2.13-2``,  ``2.13-1``,  ``2.13-0``
...
to install globally, run::
    ...
    pixi global install perl-digest-sha1

to add into an existing workspace instead, run::

    pixi add perl-digest-sha1
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname perl-digest-sha1
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_perl-digest-sha1"></div>
   <div style="width: 100%" id="platform_plot_perl-digest-sha1"></div>
   <div style="width: 100%" id="cdf_plot_perl-digest-sha1"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for perl-digest-sha1
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/perl-digest-sha1/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_perl-digest-sha1', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for perl-digest-sha1
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/perl-digest-sha1/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_perl-digest-sha1', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for perl-digest-sha1
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/perl-digest-sha1/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_perl-digest-sha1', spec);
            } catch (err) {
