...
.. conda:package:: libmems
   ...
   :depends on libgenome: ``>=1.3.1,<2.0a0``
   :depends on libmuscle: ``>=3.7,<4.0a0``
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname libmems
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_libmems"></div>
   <div style="width: 100%" id="platform_plot_libmems"></div>
   <div style="width: 100%" id="cdf_plot_libmems"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/libmems/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_libmems', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for libmems
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/libmems/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_libmems', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for libmems
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/libmems/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_libmems', spec);
            } catch (err) {
