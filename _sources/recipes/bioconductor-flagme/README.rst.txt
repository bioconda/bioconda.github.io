...
.. conda:recipe:: bioconductor-flagme
   ...
   :homepage: https://bioconductor.org/packages/3.22/bioc/html/flagme.html
   :license: LGPL (>= 2)
...
.. conda:package:: bioconductor-flagme
   ...
   :versions:
      ...
      ``1.66.0-0``,  ``1.62.0-0``,  ``1.58.0-1``,  ``1.58.0-0``,  ``1.56.0-0``,  ``1.54.0-1``,  ``1.54.0-0``,  ``1.50.0-2``,  ``1.50.0-1``,  ``1.50.0-0``,  ``1.48.0-0``,  ``1.46.0-1``,  ``1.46.0-0``,  ``1.44.0-0``,  ``1.42.0-1``,  ``1.40.0-1``,  ``1.38.1-0``,  ``1.38.0-0``,  ``1.34.0-0``
   ...
   :depends on bioconductor-gcspikelite: ``>=1.48.0,<1.49.0``
   :depends on bioconductor-gcspikelite: ``>=1.48.0,<1.49.0a0``
...
to install globally, run::
    ...
    pixi global install bioconductor-flagme

to add into an existing workspace instead, run::

    pixi add bioconductor-flagme
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-flagme
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-flagme"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-flagme"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-flagme"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-flagme
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-flagme/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-flagme', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-flagme
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-flagme/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-flagme', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-flagme
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-flagme/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-flagme', spec);
            } catch (err) {
