...
to install globally, run::
    ...
    pixi global install ucsc-pslrecalcmatch

to add into an existing workspace instead, run::

    pixi add ucsc-pslrecalcmatch
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname ucsc-pslrecalcmatch
...
.. _`ucsc-pslrecalcmatch/tags`: https://quay.io/repository/biocontainers/ucsc-pslrecalcmatch?tab=tags


.. raw:: html

   <script>
      var package = "ucsc-pslrecalcmatch";
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ucsc-pslrecalcmatch"></div>
   <div style="width: 100%" id="platform_plot_ucsc-pslrecalcmatch"></div>
   <div style="width: 100%" id="cdf_plot_ucsc-pslrecalcmatch"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ucsc-pslrecalcmatch
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-pslrecalcmatch/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ucsc-pslrecalcmatch', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ucsc-pslrecalcmatch
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-pslrecalcmatch/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ucsc-pslrecalcmatch', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ucsc-pslrecalcmatch
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-pslrecalcmatch/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ucsc-pslrecalcmatch', spec);
            } catch (err) {
