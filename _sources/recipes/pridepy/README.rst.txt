...
.. conda:package:: pridepy
   ...
   :versions:
      ...
      ``0.0.16-0``,  ``0.0.15-0``,  ``0.0.14-0``,  ``0.0.13-0``,  ``0.0.12-0``

      

   
   :depends on boto3: ``>=1.34.61,<2.0.0``
   :depends on botocore: ``>=1.34.74,<2.0.0``
   :depends on click: ``>=8.1.7,<9.0.0``
   :depends on defusedxml: ``>=0.7.1,<0.8``
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_pridepy"></div>
   <div style="width: 100%" id="platform_plot_pridepy"></div>
   <div style="width: 100%" id="cdf_plot_pridepy"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/pridepy/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_pridepy', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for pridepy
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/pridepy/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_pridepy', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for pridepy
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/pridepy/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_pridepy', spec);
            } catch (err) {
