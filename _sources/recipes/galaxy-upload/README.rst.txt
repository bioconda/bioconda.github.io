...
.. conda:package:: galaxy-upload
   ...
   :depends on bioblend: ``>=0.18.0``
   :depends on click: 
   :depends on click-option-group: 
   :depends on python: 
   :depends on rich: 
   :depends on tuspy: ``>=1.0.0``
...
to install globally, run::
    ...
    pixi global install galaxy-upload

to add into an existing workspace instead, run::

    pixi add galaxy-upload
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname galaxy-upload
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_galaxy-upload"></div>
   <div style="width: 100%" id="platform_plot_galaxy-upload"></div>
   <div style="width: 100%" id="cdf_plot_galaxy-upload"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for galaxy-upload
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/galaxy-upload/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_galaxy-upload', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for galaxy-upload
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/galaxy-upload/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_galaxy-upload', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for galaxy-upload
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/galaxy-upload/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_galaxy-upload', spec);
            } catch (err) {
