...
to install globally, run::
    ...
    pixi global install bioconductor-biodbuniprot

to add into an existing workspace instead, run::

    pixi add bioconductor-biodbuniprot
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-biodbuniprot
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-biodbuniprot"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-biodbuniprot"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-biodbuniprot"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-biodbuniprot
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-biodbuniprot/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-biodbuniprot', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-biodbuniprot
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-biodbuniprot/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-biodbuniprot', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-biodbuniprot
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-biodbuniprot/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-biodbuniprot', spec);
            } catch (err) {
