...
to install globally, run::
    ...
    pixi global install ucsc-hgloadwiggle

to add into an existing workspace instead, run::

    pixi add ucsc-hgloadwiggle
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname ucsc-hgloadwiggle
...
For e.g. docker, run::
    ...
    docker pull quay.io/biocontainers/ucsc-hgloadwiggle:<tag>

(see `ucsc-hgloadwiggle/tags`_ for valid values for ``<tag>``).
...
.. _`ucsc-hgloadwiggle/tags`: https://quay.io/repository/biocontainers/ucsc-hgloadwiggle?tab=tags


.. raw:: html

   <script>
      var package = "ucsc-hgloadwiggle";
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ucsc-hgloadwiggle"></div>
   <div style="width: 100%" id="platform_plot_ucsc-hgloadwiggle"></div>
   <div style="width: 100%" id="cdf_plot_ucsc-hgloadwiggle"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ucsc-hgloadwiggle
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-hgloadwiggle/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ucsc-hgloadwiggle', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ucsc-hgloadwiggle
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-hgloadwiggle/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ucsc-hgloadwiggle', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ucsc-hgloadwiggle
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-hgloadwiggle/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ucsc-hgloadwiggle', spec);
            } catch (err) {
