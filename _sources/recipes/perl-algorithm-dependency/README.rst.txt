...
.. conda:package:: perl-algorithm-dependency
   ...
   :depends on perl-params-util: ``>=0.31``
...
to install globally, run::
    ...
    pixi global install perl-algorithm-dependency

to add into an existing workspace instead, run::

    pixi add perl-algorithm-dependency
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname perl-algorithm-dependency
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_perl-algorithm-dependency"></div>
   <div style="width: 100%" id="platform_plot_perl-algorithm-dependency"></div>
   <div style="width: 100%" id="cdf_plot_perl-algorithm-dependency"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for perl-algorithm-dependency
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/perl-algorithm-dependency/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_perl-algorithm-dependency', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for perl-algorithm-dependency
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/perl-algorithm-dependency/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_perl-algorithm-dependency', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for perl-algorithm-dependency
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/perl-algorithm-dependency/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_perl-algorithm-dependency', spec);
            } catch (err) {
