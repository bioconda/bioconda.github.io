...
.. conda:package:: clinvar-tsv
   ...
   :depends on cattrs: 
   :depends on interval-binning: 
   :depends on logzero: 
   :depends on pysam: ``>=0.15.1``
   :depends on python: ``>=3.5``
   :depends on python-dateutil: 
   :depends on snakemake-minimal: ``>=5.3.0``
   :depends on tqdm: 
...
to install globally, run::
    ...
    pixi global install clinvar-tsv

to add into an existing workspace instead, run::

    pixi add clinvar-tsv
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname clinvar-tsv
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_clinvar-tsv"></div>
   <div style="width: 100%" id="platform_plot_clinvar-tsv"></div>
   <div style="width: 100%" id="cdf_plot_clinvar-tsv"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for clinvar-tsv
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/clinvar-tsv/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_clinvar-tsv', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for clinvar-tsv
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/clinvar-tsv/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_clinvar-tsv', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for clinvar-tsv
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/clinvar-tsv/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_clinvar-tsv', spec);
            } catch (err) {
