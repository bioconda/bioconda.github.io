...
.. conda:package:: bioutils
   ...
   :versions:
      ...
      ``0.6.1-0``,  ``0.6.0-0``,  ``0.5.7-0``,  ``0.5.6-0``,  ``0.5.5-0``
   ...
   :depends on requests: 
   :depends on urllib3: ``>=1.26,<2.dev0``
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioutils
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["0.6.1","0.6.0","0.5.7","0.5.6","0.5.5"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioutils"></div>
   <div style="width: 100%" id="platform_plot_bioutils"></div>
   <div style="width: 100%" id="cdf_plot_bioutils"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioutils/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioutils', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioutils
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioutils/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioutils', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioutils
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioutils/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioutils', spec);
            } catch (err) {
