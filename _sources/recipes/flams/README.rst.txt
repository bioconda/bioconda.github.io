...
.. conda:package:: flams
   ...
   :depends on blast: 
   :depends on certifi: ``2023.5.7``
   :depends on charset-normalizer: ``3.1.0``
   :depends on idna: ``3.4``
   :depends on numpy: ``1.24.3``
   :depends on pandas: ``2.1.2``
   :depends on python: ``>=3.10``
   :depends on python-dateutil: ``2.8.2``
   :depends on pytz: ``2023.3.post1``
   :depends on requests: ``2.31.0``
   :depends on six: ``1.16.0``
   :depends on tzdata: ``2023c``
   :depends on urllib3: ``2.0.2``
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_flams"></div>
   <div style="width: 100%" id="platform_plot_flams"></div>
   <div style="width: 100%" id="cdf_plot_flams"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/flams/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_flams', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for flams
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/flams/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_flams', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for flams
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/flams/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_flams', spec);
            } catch (err) {
