...
.. conda:recipe:: bioconductor-tenxbusdata
   ...
   :homepage: https://bioconductor.org/packages/3.22/data/experiment/html/TENxBUSData.html
...
.. conda:package:: bioconductor-tenxbusdata
   ...
   :versions:
      ...
      ``1.24.0-0``,  ``1.20.0-0``,  ``1.16.0-0``,  ``1.14.0-0``,  ``1.12.0-0``,  ``1.8.0-1``,  ``1.8.0-0``,  ``1.6.0-0``,  ``1.4.0-1``,  ``1.4.0-0``,  ``1.2.1-0``,  ``1.0.0-0``
...
to install globally, run::
    ...
    pixi global install bioconductor-tenxbusdata

to add into an existing workspace instead, run::

    pixi add bioconductor-tenxbusdata
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-tenxbusdata
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-tenxbusdata"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-tenxbusdata"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-tenxbusdata"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-tenxbusdata
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-tenxbusdata/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-tenxbusdata', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-tenxbusdata
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-tenxbusdata/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-tenxbusdata', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-tenxbusdata
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-tenxbusdata/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-tenxbusdata', spec);
            } catch (err) {
