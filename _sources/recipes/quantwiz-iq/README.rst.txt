...
.. conda:package:: quantwiz-iq
   ...
   :depends on perl: 
   :depends on perl-math-matrix: ``0.8.*``
   :depends on perl-math-matrixreal: ``2.13.*``
   :depends on perl-mime-base64: ``3.15.*``
   :depends on perl-xml-twig: ``3.52.*``
...
to install globally, run::
    ...
    pixi global install quantwiz-iq

to add into an existing workspace instead, run::

    pixi add quantwiz-iq
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname quantwiz-iq
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_quantwiz-iq"></div>
   <div style="width: 100%" id="platform_plot_quantwiz-iq"></div>
   <div style="width: 100%" id="cdf_plot_quantwiz-iq"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for quantwiz-iq
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/quantwiz-iq/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_quantwiz-iq', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for quantwiz-iq
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/quantwiz-iq/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_quantwiz-iq', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for quantwiz-iq
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/quantwiz-iq/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_quantwiz-iq', spec);
            } catch (err) {
