...
.. conda:package:: ucsc-genepredtobed
   ...
   :versions:
      ...
      .. raw:: html
         ...
         <details><summary><span class="truncated-version-list"><code>482-0</code>,  <code>469-1</code>,  <code>469-0</code>,  <code>447-1</code>,  <code>447-0</code>,  <code>377-5</code>,  <code>377-4</code>,  <code>377-3</code>,  <code>377-2</code>,  </span></summary>
      

      ``482-0``,  ``469-1``,  ``469-0``,  ``447-1``,  ``447-0``,  ``377-5``,  ``377-4``,  ``377-3``,  ``377-2``,  ``377-1``,  ``366-1``,  ``366-0``,  ``357-1``,  ``357-0``,  ``332-0``,  ``324-0``
...
to install globally, run::
    ...
    pixi global install ucsc-genepredtobed

to add into an existing workspace instead, run::

    pixi add ucsc-genepredtobed
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname ucsc-genepredtobed
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ucsc-genepredtobed"></div>
   <div style="width: 100%" id="platform_plot_ucsc-genepredtobed"></div>
   <div style="width: 100%" id="cdf_plot_ucsc-genepredtobed"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ucsc-genepredtobed
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-genepredtobed/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ucsc-genepredtobed', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ucsc-genepredtobed
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-genepredtobed/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ucsc-genepredtobed', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ucsc-genepredtobed
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-genepredtobed/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ucsc-genepredtobed', spec);
            } catch (err) {
