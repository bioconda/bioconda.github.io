...
.. conda:package:: spestimator
   ...
   :versions:
      ...
      ``0.3.0.237-0``,  ``0.3.0.236-0``,  ``0.3.0.235-0``,  ``0.3.0.234-0``,  ``0.3.0.233-0``,  ``0.1.0.232-0``

      

   
   :depends on blast: 
   :depends on ncbi-datasets-pyclient: 
   :depends on pandas: 
...
to install globally, run::
    ...
    pixi global install spestimator

to add into an existing workspace instead, run::

    pixi add spestimator
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install spestimator

Alternatively, to install into a new environment, run::

    conda create -n envname spestimator
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["0.3.0.237","0.3.0.236","0.3.0.235","0.3.0.234","0.3.0.233"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_spestimator"></div>
   <div style="width: 100%" id="platform_plot_spestimator"></div>
   <div style="width: 100%" id="cdf_plot_spestimator"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for spestimator
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/spestimator/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_spestimator', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for spestimator
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/spestimator/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_spestimator', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for spestimator
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/spestimator/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_spestimator', spec);
            } catch (err) {
