...
.. conda:package:: strainflye
   ...
   :depends on bcftools: ``>=1.7,<1.10``
   :depends on click: ``>=8.0``
   :depends on htslib: ``>=1.9,<1.10.0a0``
   :depends on minimap2: ``>=2.10``
   :depends on networkx: 
   :depends on numpy: 
...
to install globally, run::
    ...
    pixi global install strainflye
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname strainflye
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_strainflye"></div>
   <div style="width: 100%" id="platform_plot_strainflye"></div>
   <div style="width: 100%" id="cdf_plot_strainflye"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/strainflye/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_strainflye', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for strainflye
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/strainflye/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_strainflye', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for strainflye
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/strainflye/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_strainflye', spec);
            } catch (err) {
