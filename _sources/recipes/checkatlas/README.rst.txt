...
.. conda:package:: checkatlas
   ...
   :versions:
      ...
      ``0.8.2-0``,  ``0.8.0-0``,  ``0.7.1-0``,  ``0.6.0-0``,  ``0.5.6-0``,  ``0.4.17-0``,  ``0.4.13-0``,  ``0.4.7-0``,  ``0.4.2-0``
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["0.8.2","0.8.0","0.7.1","0.6.0","0.5.6"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_checkatlas"></div>
   <div style="width: 100%" id="platform_plot_checkatlas"></div>
   <div style="width: 100%" id="cdf_plot_checkatlas"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/checkatlas/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_checkatlas', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for checkatlas
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/checkatlas/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_checkatlas', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for checkatlas
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/checkatlas/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_checkatlas', spec);
            } catch (err) {
