...
to install globally, run::
    ...
    pixi global install ucsc-bedgeneparts

to add into an existing workspace instead, run::

    pixi add ucsc-bedgeneparts
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname ucsc-bedgeneparts
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ucsc-bedgeneparts"></div>
   <div style="width: 100%" id="platform_plot_ucsc-bedgeneparts"></div>
   <div style="width: 100%" id="cdf_plot_ucsc-bedgeneparts"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ucsc-bedgeneparts
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-bedgeneparts/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ucsc-bedgeneparts', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ucsc-bedgeneparts
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-bedgeneparts/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ucsc-bedgeneparts', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ucsc-bedgeneparts
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-bedgeneparts/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ucsc-bedgeneparts', spec);
            } catch (err) {
