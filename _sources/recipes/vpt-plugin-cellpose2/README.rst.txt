...
.. conda:package:: vpt-plugin-cellpose2
   ...
   :depends on cellpose: ``2.2.3``
   :depends on numpy: ``1.24.3``
   :depends on onnx: ``>=1.12.0,<1.15.0``
   :depends on openvino: ``>=2023.2.0``
...
to install globally, run::
    ...
    pixi global install vpt-plugin-cellpose2

to add into an existing workspace instead, run::

    pixi add vpt-plugin-cellpose2
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install vpt-plugin-cellpose2

Alternatively, to install into a new environment, run::

    conda create -n envname vpt-plugin-cellpose2
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_vpt-plugin-cellpose2"></div>
   <div style="width: 100%" id="platform_plot_vpt-plugin-cellpose2"></div>
   <div style="width: 100%" id="cdf_plot_vpt-plugin-cellpose2"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for vpt-plugin-cellpose2
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/vpt-plugin-cellpose2/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_vpt-plugin-cellpose2', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for vpt-plugin-cellpose2
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/vpt-plugin-cellpose2/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_vpt-plugin-cellpose2', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for vpt-plugin-cellpose2
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/vpt-plugin-cellpose2/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_vpt-plugin-cellpose2', spec);
            } catch (err) {
