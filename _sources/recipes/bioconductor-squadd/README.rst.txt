...
.. conda:recipe:: bioconductor-squadd
   ...
   :homepage: https://bioconductor.org/packages/3.18/bioc/html/SQUADD.html
   :license: GPL (>=2)
...
.. conda:package:: bioconductor-squadd
   ...
   :versions:
      ...
      ``1.52.0-0``,  ``1.50.0-0``,  ``1.48.0-0``,  ``1.44.0-0``,  ``1.42.0-0``,  ``1.40.0-1``,  ``1.40.0-0``,  ``1.38.0-0``,  ``1.36.0-0``,  ``1.34.0-1``,  ``1.34.0-0``,  ``1.32.0-0``,  ``1.30.0-0``,  ``1.28.0-0``,  ``1.26.0-0``
...
to install globally, run::
    ...
    pixi global install bioconductor-squadd

to add into an existing workspace instead, run::

    pixi add bioconductor-squadd
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-squadd
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-squadd"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-squadd"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-squadd"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-squadd
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-squadd/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-squadd', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-squadd
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-squadd/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-squadd', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-squadd
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-squadd/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-squadd', spec);
            } catch (err) {
