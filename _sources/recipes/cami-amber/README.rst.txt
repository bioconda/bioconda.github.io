...
.. conda:package:: cami-amber
   ...
   :versions:
      ...
      ``2.0.7-0``,  ``2.0.6-0``,  ``2.0.5-0``,  ``2.0.4-1``,  ``2.0.4-0``,  ``2.0.3-0``,  ``2.0.2-0``,  ``2.0.1-0``,  ``2.0.0.post0-0``

      

   
   :depends on biopython: ``>=1.84``
   :depends on bokeh: ``>=3.5.1``
   :depends on jinja2: ``>=3.1.4``
   :depends on matplotlib-base: ``>=3.8.4``
   :depends on numpy: ``>=2.0.1``
...
to install globally, run::
    ...
    pixi global install cami-amber
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname cami-amber
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_cami-amber"></div>
   <div style="width: 100%" id="platform_plot_cami-amber"></div>
   <div style="width: 100%" id="cdf_plot_cami-amber"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/cami-amber/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_cami-amber', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for cami-amber
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/cami-amber/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_cami-amber', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for cami-amber
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/cami-amber/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_cami-amber', spec);
            } catch (err) {
