...
.. conda:package:: fineradstructure
   ...
   :versions:
      ...
      ``0.3.2r109-7``,  ``0.3.2r109-6``,  ``0.3.2r109-5``,  ``0.3.2r109-4``,  ``0.3.2r109-3``,  ``0.3.2r109-2``,  ``0.3.2r109-1``,  ``0.3.2r109-0``
...
to install globally, run::
    ...
    pixi global install fineradstructure

to add into an existing workspace instead, run::

    pixi add fineradstructure
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname fineradstructure
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_fineradstructure"></div>
   <div style="width: 100%" id="platform_plot_fineradstructure"></div>
   <div style="width: 100%" id="cdf_plot_fineradstructure"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for fineradstructure
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/fineradstructure/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_fineradstructure', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for fineradstructure
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/fineradstructure/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_fineradstructure', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for fineradstructure
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/fineradstructure/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_fineradstructure', spec);
            } catch (err) {
