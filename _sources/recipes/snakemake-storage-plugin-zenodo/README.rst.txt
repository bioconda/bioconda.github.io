...
.. conda:package:: snakemake-storage-plugin-zenodo
   ...
   :depends on snakemake-interface-common: ``>=1.14.4,<2.0.0``
...
to install globally, run::
    ...
    pixi global install snakemake-storage-plugin-zenodo

to add into an existing workspace instead, run::

    pixi add snakemake-storage-plugin-zenodo
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname snakemake-storage-plugin-zenodo
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_snakemake-storage-plugin-zenodo"></div>
   <div style="width: 100%" id="platform_plot_snakemake-storage-plugin-zenodo"></div>
   <div style="width: 100%" id="cdf_plot_snakemake-storage-plugin-zenodo"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for snakemake-storage-plugin-zenodo
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-storage-plugin-zenodo/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_snakemake-storage-plugin-zenodo', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for snakemake-storage-plugin-zenodo
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-storage-plugin-zenodo/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_snakemake-storage-plugin-zenodo', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for snakemake-storage-plugin-zenodo
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-storage-plugin-zenodo/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_snakemake-storage-plugin-zenodo', spec);
            } catch (err) {
