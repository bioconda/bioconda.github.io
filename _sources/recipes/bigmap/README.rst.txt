...
.. conda:package:: bigmap
   ...
   :depends on bioconductor-biomformat: ``>=1.30.0``
   :depends on bioconductor-metagenomeseq: ``>=1.43.0``
   :depends on biom-format: ``>=2.1.16``
   :depends on biopython: ``>=1.81``
   :depends on bowtie2: ``>=2.5.4``
   :depends on cxx-compiler: ``>=1.9.0``
   :depends on hmmer: ``>=3.4``
   :depends on importlib_resources: ``>=6.5.2``
   :depends on mash: ``>=2.3``
   :depends on numpy: ``<2``
   :depends on pandas: ``>=2.3.3``
   :depends on python: ``>=3.11.11``
   :depends on rpy2: ``>=3.5.11``
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install bigmap

Alternatively, to install into a new environment, run::

    conda create -n envname bigmap
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bigmap"></div>
   <div style="width: 100%" id="platform_plot_bigmap"></div>
   <div style="width: 100%" id="cdf_plot_bigmap"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bigmap/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bigmap', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bigmap
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bigmap/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bigmap', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bigmap
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bigmap/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bigmap', spec);
            } catch (err) {
