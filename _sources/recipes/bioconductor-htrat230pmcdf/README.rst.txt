...
.. title:: Package Recipe 'bioconductor-htrat230pmcdf'
.. highlight: bash
...
.. conda:recipe:: bioconductor-htrat230pmcdf
   ...
   :homepage: https://bioconductor.org/packages/3.22/data/annotation/html/htrat230pmcdf.html
   :license: LGPL
...
to install globally, run::
    ...
    pixi global install bioconductor-htrat230pmcdf

to add into an existing workspace instead, run::

    pixi add bioconductor-htrat230pmcdf
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install bioconductor-htrat230pmcdf

Alternatively, to install into a new environment, run::

    conda create -n envname bioconductor-htrat230pmcdf
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-htrat230pmcdf"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-htrat230pmcdf"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-htrat230pmcdf"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-htrat230pmcdf
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-htrat230pmcdf/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-htrat230pmcdf', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-htrat230pmcdf
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-htrat230pmcdf/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-htrat230pmcdf', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-htrat230pmcdf
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-htrat230pmcdf/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-htrat230pmcdf', spec);
            } catch (err) {
...
Render an |install-with-bioconda| badge with the following MarkDown::
   ...
   [![install with bioconda](https://img.shields.io/badge/install%20with-bioconda-brightgreen.svg?style=flat)](http://bioconda.github.io/recipes/bioconductor-htrat230pmcdf/README.html)

.. |install-with-bioconda| image:: https://img.shields.io/badge/install%20with-bioconda-brightgreen.svg?style=flat
   :target: http://bioconda.github.io/recipes/bioconductor-htrat230pmcdf/README.html