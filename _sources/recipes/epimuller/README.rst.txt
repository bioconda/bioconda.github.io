...
.. conda:package:: epimuller
   ...
   :depends on cairosvg: ``>=2.4``
   :depends on ete3: ``>=3.1``
   :depends on pycairo: ``>=1.20``
   :depends on python: ``>3.7``
   :depends on svgwrite: ``>=1.4``
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname epimuller
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_epimuller"></div>
   <div style="width: 100%" id="platform_plot_epimuller"></div>
   <div style="width: 100%" id="cdf_plot_epimuller"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/epimuller/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_epimuller', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for epimuller
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/epimuller/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_epimuller', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for epimuller
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/epimuller/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_epimuller', spec);
            } catch (err) {
