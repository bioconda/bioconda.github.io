...
.. conda:recipe:: bioconductor-deepbluer
   ...
   :homepage: https://bioconductor.org/packages/3.18/bioc/html/DeepBlueR.html
   :license: GPL (>=2.0)
...
.. conda:package:: bioconductor-deepbluer
   ...
   :versions:
      ...
      ``1.24.0-0``,  ``1.20.0-0``,  ``1.18.0-0``,  ``1.16.0-1``,  ``1.16.0-0``,  ``1.14.0-0``,  ``1.12.0-0``,  ``1.10.0-1``,  ``1.8.0-0``,  ``1.6.0-0``,  ``1.4.1-0``
   ...
   :depends on r-diffr: 
   :depends on r-dplyr: 
   :depends on r-filehash: 
   :depends on r-foreach: 
   :depends on r-r.utils: 
   :depends on r-rcurl: 
   :depends on r-rjson: 
   :depends on r-settings: 
...
to install globally, run::
    ...
    pixi global install bioconductor-deepbluer

to add into an existing workspace instead, run::

    pixi add bioconductor-deepbluer
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-deepbluer
...
.. raw:: html
   ...
   <script>
      var package = "bioconductor-deepbluer";
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-deepbluer"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-deepbluer"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-deepbluer"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-deepbluer
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-deepbluer/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-deepbluer', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-deepbluer
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-deepbluer/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-deepbluer', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-deepbluer
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-deepbluer/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-deepbluer', spec);
            } catch (err) {
