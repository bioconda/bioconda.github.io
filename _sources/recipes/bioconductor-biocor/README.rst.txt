...
.. conda:package:: bioconductor-biocor
   ...
   :versions:
      ...
      ``1.34.0-0``,  ``1.30.0-0``,  ``1.26.0-0``,  ``1.24.0-0``,  ``1.22.0-0``,  ``1.18.0-0``,  ``1.16.0-0``,  ``1.14.0-1``,  ``1.14.0-0``,  ``1.12.0-0``,  ``1.10.0-0``,  ``1.8.1-0``,  ``1.6.0-0``
...
to install globally, run::
    ...
    pixi global install bioconductor-biocor

to add into an existing workspace instead, run::

    pixi add bioconductor-biocor
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-biocor
...
.. raw:: html
   ...
   <script>
      var package = "bioconductor-biocor";
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-biocor"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-biocor"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-biocor"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-biocor
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-biocor/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-biocor', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-biocor
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-biocor/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-biocor', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-biocor
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-biocor/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-biocor', spec);
            } catch (err) {
