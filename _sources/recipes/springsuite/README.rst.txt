...
to install globally, run::
    ...
    pixi global install springsuite

to add into an existing workspace instead, run::

    pixi add springsuite
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname springsuite
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_springsuite"></div>
   <div style="width: 100%" id="platform_plot_springsuite"></div>
   <div style="width: 100%" id="cdf_plot_springsuite"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for springsuite
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/springsuite/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_springsuite', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for springsuite
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/springsuite/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_springsuite', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for springsuite
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/springsuite/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_springsuite', spec);
            } catch (err) {
