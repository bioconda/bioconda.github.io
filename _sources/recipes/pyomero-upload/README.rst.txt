...
.. conda:package:: pyomero-upload
   ...
   :depends on python-omero: ``5.6.2.*``
   :depends on pyyaml: 
   :depends on requests: 
   :depends on scipy: 
   :depends on zeroc-ice: ``3.6.*``
...
to install globally, run::
    ...
    pixi global install pyomero-upload

to add into an existing workspace instead, run::

    pixi add pyomero-upload
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname pyomero-upload
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_pyomero-upload"></div>
   <div style="width: 100%" id="platform_plot_pyomero-upload"></div>
   <div style="width: 100%" id="cdf_plot_pyomero-upload"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for pyomero-upload
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/pyomero-upload/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_pyomero-upload', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for pyomero-upload
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/pyomero-upload/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_pyomero-upload', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for pyomero-upload
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/pyomero-upload/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_pyomero-upload', spec);
            } catch (err) {
