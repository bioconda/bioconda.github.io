...
.. conda:recipe:: bioconductor-mlseq
   ...
   :homepage: https://bioconductor.org/packages/3.22/bioc/html/MLSeq.html
   :license: GPL(>=2)
...
.. conda:package:: bioconductor-mlseq
   ...
   :versions:
      ...
      ``2.28.0-0``,  ``2.24.0-0``,  ``2.20.0-0``,  ``2.18.0-0``,  ``2.16.0-0``,  ``2.12.0-0``,  ``2.10.0-0``,  ``2.8.0-1``,  ``2.8.0-0``,  ``2.6.0-0``,  ``2.4.0-0``,  ``2.2.1-1``,  ``2.0.0-0``,  ``1.20.3-0``,  ``1.18.0-0``
   ...
   :depends on bioconductor-sseq: ``>=1.48.0,<1.49.0``
...
to install globally, run::
    ...
    pixi global install bioconductor-mlseq

to add into an existing workspace instead, run::

    pixi add bioconductor-mlseq
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-mlseq
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-mlseq"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-mlseq"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-mlseq"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-mlseq
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-mlseq/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-mlseq', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-mlseq
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-mlseq/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-mlseq', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-mlseq
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-mlseq/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-mlseq', spec);
            } catch (err) {
