...
.. conda:package:: epimetheus-py
   ...
   :versions:
      ...
      ``0.7.9-0``,  ``0.7.7-0``,  ``0.7.6-0``,  ``0.7.5-0``,  ``0.7.4-0``,  ``0.7.2-0``,  ``0.6.2-0``
...
to install globally, run::
    ...
    pixi global install epimetheus-py

to add into an existing workspace instead, run::

    pixi add epimetheus-py
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install epimetheus-py

Alternatively, to install into a new environment, run::

    conda create -n envname epimetheus-py
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["0.7.9","0.7.7","0.7.6","0.7.5","0.7.4"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_epimetheus-py"></div>
   <div style="width: 100%" id="platform_plot_epimetheus-py"></div>
   <div style="width: 100%" id="cdf_plot_epimetheus-py"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for epimetheus-py
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/epimetheus-py/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_epimetheus-py', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for epimetheus-py
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/epimetheus-py/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_epimetheus-py', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for epimetheus-py
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/epimetheus-py/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_epimetheus-py', spec);
            } catch (err) {
