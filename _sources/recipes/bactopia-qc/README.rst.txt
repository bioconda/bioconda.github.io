...
.. conda:package:: bactopia-qc
   ...
   :depends on gsl: ``2.6.*``
   :depends on lighter: ``>=1.1.2``
   :depends on nanoplot: ``>=1.41.6``
...
to install globally, run::
    ...
    pixi global install bactopia-qc

to add into an existing workspace instead, run::

    pixi add bactopia-qc
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bactopia-qc
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bactopia-qc"></div>
   <div style="width: 100%" id="platform_plot_bactopia-qc"></div>
   <div style="width: 100%" id="cdf_plot_bactopia-qc"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bactopia-qc
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bactopia-qc/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bactopia-qc', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bactopia-qc
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bactopia-qc/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bactopia-qc', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bactopia-qc
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bactopia-qc/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bactopia-qc', spec);
            } catch (err) {
