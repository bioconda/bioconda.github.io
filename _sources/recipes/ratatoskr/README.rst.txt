...
.. conda:package:: ratatoskr
   ...
   :versions:
      ...
      ``1.0.10-0``,  ``1.0.9-0``,  ``1.0.8-0``,  ``1.0.6-0``,  ``1.0.5-0``,  ``1.0.4-0``,  ``1.0.2-0``
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["1.0.10","1.0.9","1.0.8","1.0.6","1.0.5"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ratatoskr"></div>
   <div style="width: 100%" id="platform_plot_ratatoskr"></div>
   <div style="width: 100%" id="cdf_plot_ratatoskr"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ratatoskr/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ratatoskr', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ratatoskr
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ratatoskr/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ratatoskr', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ratatoskr
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ratatoskr/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ratatoskr', spec);
            } catch (err) {
