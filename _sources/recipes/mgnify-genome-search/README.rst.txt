...
to install globally, run::
    ...
    pixi global install mgnify-genome-search

to add into an existing workspace instead, run::

    pixi add mgnify-genome-search
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install mgnify-genome-search

Alternatively, to install into a new environment, run::

    conda create -n envname mgnify-genome-search
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_mgnify-genome-search"></div>
   <div style="width: 100%" id="platform_plot_mgnify-genome-search"></div>
   <div style="width: 100%" id="cdf_plot_mgnify-genome-search"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for mgnify-genome-search
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/mgnify-genome-search/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_mgnify-genome-search', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for mgnify-genome-search
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/mgnify-genome-search/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_mgnify-genome-search', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for mgnify-genome-search
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/mgnify-genome-search/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_mgnify-genome-search', spec);
            } catch (err) {
