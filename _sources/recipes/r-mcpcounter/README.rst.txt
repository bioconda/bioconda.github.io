...
.. conda:package:: r-mcpcounter
   ...
   :versions:
      ...
      ``1.1.0-6``,  ``1.1.0-5``,  ``1.1.0-4``,  ``1.1.0-3``,  ``1.1.0-2``,  ``1.1.0-1``,  ``1.1.0-0``,  ``0-0``

      

   
   :depends on r-base: ``>=4.4,<4.5.0a0``
   :depends on r-curl: ``>=2.6``
...
to install globally, run::
    ...
    pixi global install r-mcpcounter

to add into an existing workspace instead, run::

    pixi add r-mcpcounter
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname r-mcpcounter
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_r-mcpcounter"></div>
   <div style="width: 100%" id="platform_plot_r-mcpcounter"></div>
   <div style="width: 100%" id="cdf_plot_r-mcpcounter"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for r-mcpcounter
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/r-mcpcounter/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_r-mcpcounter', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for r-mcpcounter
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/r-mcpcounter/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_r-mcpcounter', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for r-mcpcounter
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/r-mcpcounter/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_r-mcpcounter', spec);
            } catch (err) {
