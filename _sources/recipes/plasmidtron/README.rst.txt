...
.. conda:package:: plasmidtron
   ...
   :depends on kmc: ``>=2.3.0``
   :depends on matplotlib: 
   :depends on pyfastaq: ``>=3.12.0``
   :depends on python: ``>=3``
   :depends on spades: ``>=3.9.0``
...
to install globally, run::
    ...
    pixi global install plasmidtron

to add into an existing workspace instead, run::

    pixi add plasmidtron
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname plasmidtron
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_plasmidtron"></div>
   <div style="width: 100%" id="platform_plot_plasmidtron"></div>
   <div style="width: 100%" id="cdf_plot_plasmidtron"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for plasmidtron
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/plasmidtron/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_plasmidtron', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for plasmidtron
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/plasmidtron/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_plasmidtron', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for plasmidtron
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/plasmidtron/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_plasmidtron', spec);
            } catch (err) {
