...
.. conda:package:: autobigs-cli
   ...
   :versions:
      ...
      ``0.6.5-0``,  ``0.6.3-0``,  ``0.6.2-0``,  ``0.6.0-0``,  ``0.5.0-0``,  ``0.4.2-0``

      

   
   :depends on autobigs-engine: ``0.14.*``
...
to install globally, run::
    ...
    pixi global install autobigs-cli

to add into an existing workspace instead, run::

    pixi add autobigs-cli
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install autobigs-cli

Alternatively, to install into a new environment, run::

    conda create -n envname autobigs-cli
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["0.6.5","0.6.3","0.6.2","0.6.0","0.5.0"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_autobigs-cli"></div>
   <div style="width: 100%" id="platform_plot_autobigs-cli"></div>
   <div style="width: 100%" id="cdf_plot_autobigs-cli"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for autobigs-cli
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/autobigs-cli/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_autobigs-cli', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for autobigs-cli
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/autobigs-cli/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_autobigs-cli', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for autobigs-cli
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/autobigs-cli/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_autobigs-cli', spec);
            } catch (err) {
