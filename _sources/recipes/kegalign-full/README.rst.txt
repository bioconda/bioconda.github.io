...
.. conda:package:: kegalign-full
   ...
   :versions:
      ...
      ``0.3.2-0``,  ``0.3.1-0``,  ``0.3.0-0``,  ``0.2.2.14-0``,  ``0.2.1.13-0``,  ``0.1.2.9-0``,  ``0.1.2.8-0``,  ``0.1.2.7-0``

      

   
   :depends on kegalign: ``0.3.2.*``
   :depends on lastz: 
   :depends on mbuffer: 
   :depends on samtools: 
...
to install globally, run::
    ...
    pixi global install kegalign-full

to add into an existing workspace instead, run::

    pixi add kegalign-full
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install kegalign-full

Alternatively, to install into a new environment, run::

    conda create -n envname kegalign-full
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["0.3.2","0.3.1","0.3.0","0.2.2.14","0.2.1.13"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_kegalign-full"></div>
   <div style="width: 100%" id="platform_plot_kegalign-full"></div>
   <div style="width: 100%" id="cdf_plot_kegalign-full"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for kegalign-full
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/kegalign-full/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_kegalign-full', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for kegalign-full
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/kegalign-full/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_kegalign-full', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for kegalign-full
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/kegalign-full/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_kegalign-full', spec);
            } catch (err) {
