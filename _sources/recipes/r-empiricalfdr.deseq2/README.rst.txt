...
.. conda:package:: r-empiricalfdr.deseq2
   ...
   :versions:
      ...
      ``1.0.3-11``,  ``1.0.3-10``,  ``1.0.3-9``,  ``1.0.3-8``,  ``1.0.3-7``,  ``1.0.3-6``,  ``1.0.3-5``,  ``1.0.3-4``,  ``1.0.3-3``,  ``1.0.3-2``,  ``1.0.3-0``
...
to install globally, run::
    ...
    pixi global install r-empiricalfdr.deseq2

to add into an existing workspace instead, run::

    pixi add r-empiricalfdr.deseq2
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname r-empiricalfdr.deseq2
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_r-empiricalfdr.deseq2"></div>
   <div style="width: 100%" id="platform_plot_r-empiricalfdr.deseq2"></div>
   <div style="width: 100%" id="cdf_plot_r-empiricalfdr.deseq2"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for r-empiricalfdr.deseq2
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/r-empiricalfdr.deseq2/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_r-empiricalfdr.deseq2', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for r-empiricalfdr.deseq2
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/r-empiricalfdr.deseq2/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_r-empiricalfdr.deseq2', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for r-empiricalfdr.deseq2
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/r-empiricalfdr.deseq2/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_r-empiricalfdr.deseq2', spec);
            } catch (err) {
