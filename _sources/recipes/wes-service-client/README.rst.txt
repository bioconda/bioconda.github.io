...
.. conda:package:: wes-service-client
   ...
   :depends on future: 
   :depends on python: 
   :depends on schema-salad: ``>=2.6.20170927145003,<3.0``
...
to install globally, run::
    ...
    pixi global install wes-service-client

to add into an existing workspace instead, run::

    pixi add wes-service-client
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname wes-service-client
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_wes-service-client"></div>
   <div style="width: 100%" id="platform_plot_wes-service-client"></div>
   <div style="width: 100%" id="cdf_plot_wes-service-client"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for wes-service-client
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/wes-service-client/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_wes-service-client', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for wes-service-client
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/wes-service-client/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_wes-service-client', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for wes-service-client
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/wes-service-client/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_wes-service-client', spec);
            } catch (err) {
