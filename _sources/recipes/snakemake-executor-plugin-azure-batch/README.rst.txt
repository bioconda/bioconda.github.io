...
to install globally, run::
    ...
    pixi global install snakemake-executor-plugin-azure-batch

to add into an existing workspace instead, run::

    pixi add snakemake-executor-plugin-azure-batch
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install snakemake-executor-plugin-azure-batch

Alternatively, to install into a new environment, run::

    conda create -n envname snakemake-executor-plugin-azure-batch
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_snakemake-executor-plugin-azure-batch"></div>
   <div style="width: 100%" id="platform_plot_snakemake-executor-plugin-azure-batch"></div>
   <div style="width: 100%" id="cdf_plot_snakemake-executor-plugin-azure-batch"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for snakemake-executor-plugin-azure-batch
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-executor-plugin-azure-batch/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_snakemake-executor-plugin-azure-batch', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for snakemake-executor-plugin-azure-batch
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-executor-plugin-azure-batch/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_snakemake-executor-plugin-azure-batch', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for snakemake-executor-plugin-azure-batch
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-executor-plugin-azure-batch/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_snakemake-executor-plugin-azure-batch', spec);
            } catch (err) {
