...
.. conda:package:: snakemake-storage-plugin-ftp
   ...
   :depends on ftputil: ``>=5.0.4,<6.0.0``
   :depends on python: ``>=3.11.0,<4.0.0``
   :depends on snakemake-interface-common: ``>=1.15.1,<2.0.0``
...
to install globally, run::
    ...
    pixi global install snakemake-storage-plugin-ftp

to add into an existing workspace instead, run::

    pixi add snakemake-storage-plugin-ftp
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install snakemake-storage-plugin-ftp

Alternatively, to install into a new environment, run::

    conda create -n envname snakemake-storage-plugin-ftp
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_snakemake-storage-plugin-ftp"></div>
   <div style="width: 100%" id="platform_plot_snakemake-storage-plugin-ftp"></div>
   <div style="width: 100%" id="cdf_plot_snakemake-storage-plugin-ftp"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for snakemake-storage-plugin-ftp
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-storage-plugin-ftp/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_snakemake-storage-plugin-ftp', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for snakemake-storage-plugin-ftp
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-storage-plugin-ftp/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_snakemake-storage-plugin-ftp', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for snakemake-storage-plugin-ftp
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/snakemake-storage-plugin-ftp/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_snakemake-storage-plugin-ftp', spec);
            } catch (err) {
