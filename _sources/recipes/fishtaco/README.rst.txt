...
.. conda:package:: fishtaco
   ...
   :depends on musicc: ``>=1.0.2``
   :depends on numpy: ``>=1.6.1``
   :depends on pandas: ``>=0.14``
   :depends on python: 
   :depends on scikit-learn: ``>=0.15.2,<0.18.0a0``
   :depends on scipy: ``>=0.9``
   :depends on statsmodels: ``>=0.5.0``
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname fishtaco
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_fishtaco"></div>
   <div style="width: 100%" id="platform_plot_fishtaco"></div>
   <div style="width: 100%" id="cdf_plot_fishtaco"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/fishtaco/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_fishtaco', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for fishtaco
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/fishtaco/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_fishtaco', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for fishtaco
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/fishtaco/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_fishtaco', spec);
            } catch (err) {
