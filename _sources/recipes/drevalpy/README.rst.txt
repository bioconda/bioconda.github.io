...
.. conda:recipe:: drevalpy
   ...
   Drug response evaluation of drug response prediction models in a fair setting
   ...
   :links: https: :https:`//doi.org/10.1038/s41467-026-72903-w`
...
.. conda:package:: drevalpy
   ...
   :depends on typer: ``>=0.26``
   :depends on wandb: ``>=0.24.0``
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_drevalpy"></div>
   <div style="width: 100%" id="platform_plot_drevalpy"></div>
   <div style="width: 100%" id="cdf_plot_drevalpy"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/drevalpy/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_drevalpy', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for drevalpy
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/drevalpy/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_drevalpy', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for drevalpy
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/drevalpy/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_drevalpy', spec);
            } catch (err) {
