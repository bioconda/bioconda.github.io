...
to install globally, run::
    ...
    pixi global install ucsc-hgloadout

to add into an existing workspace instead, run::

    pixi add ucsc-hgloadout
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname ucsc-hgloadout
...
For e.g. docker, run::
    ...
    docker pull quay.io/biocontainers/ucsc-hgloadout:<tag>

(see `ucsc-hgloadout/tags`_ for valid values for ``<tag>``).
...
.. _`ucsc-hgloadout/tags`: https://quay.io/repository/biocontainers/ucsc-hgloadout?tab=tags


.. raw:: html

   <script>
      var package = "ucsc-hgloadout";
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ucsc-hgloadout"></div>
   <div style="width: 100%" id="platform_plot_ucsc-hgloadout"></div>
   <div style="width: 100%" id="cdf_plot_ucsc-hgloadout"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ucsc-hgloadout
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-hgloadout/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ucsc-hgloadout', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ucsc-hgloadout
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-hgloadout/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ucsc-hgloadout', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ucsc-hgloadout
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-hgloadout/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ucsc-hgloadout', spec);
            } catch (err) {
