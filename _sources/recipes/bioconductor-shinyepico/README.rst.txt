...
.. conda:recipe:: bioconductor-shinyepico
   ...
   :homepage: https://bioconductor.org/packages/3.22/bioc/html/shinyepico.html
...
.. conda:package:: bioconductor-shinyepico
   ...
   :depends on r-data.table: ``>=1.13.0``
   :depends on r-doparallel: ``>=1.0.0``
   :depends on r-dplyr: ``>=1.0.9``
   :depends on r-dt: ``>=0.15.0``
   :depends on r-foreach: ``>=1.5.0``
   :depends on r-ggplot2: ``>=3.3.0``
   :depends on r-gplots: ``>=3.0.0``
   :depends on r-heatmaply: ``>=1.1.0``
   :depends on r-plotly: ``>=4.9.2``
   :depends on r-reshape2: ``>=1.4.0``
   :depends on r-rlang: ``>=1.0.2``
   :depends on r-rmarkdown: ``>=2.3.0``
   :depends on r-shiny: ``>=1.5.0``
   :depends on r-shinycssloaders: ``>=0.3.0``
   :depends on r-shinyjs: ``>=1.1.0``
   :depends on r-shinythemes: ``>=1.1.0``
   :depends on r-shinywidgets: ``>=0.5.0``
   :depends on r-statmod: ``>=1.4.0``
   :depends on r-tidyr: ``>=1.2.0``
   :depends on r-zip: ``>=2.1.0``
...
to install globally, run::
    ...
    pixi global install bioconductor-shinyepico

to add into an existing workspace instead, run::

    pixi add bioconductor-shinyepico
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-shinyepico
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-shinyepico"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-shinyepico"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-shinyepico"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-shinyepico
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-shinyepico/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-shinyepico', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-shinyepico
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-shinyepico/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-shinyepico', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-shinyepico
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-shinyepico/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-shinyepico', spec);
            } catch (err) {
