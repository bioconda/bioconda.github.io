...
.. conda:package:: ucsc-bigpsltopsl
   ...
   :versions:
      ...
      ``482-0``,  ``469-1``,  ``469-0``,  ``465-0``,  ``377-3``,  ``377-2``,  ``377-1``,  ``377-0``,  ``366-0``,  ``357-2``,  ``357-1``,  ``357-0``,  ``332-0``
...
to install globally, run::
    ...
    pixi global install ucsc-bigpsltopsl

to add into an existing workspace instead, run::

    pixi add ucsc-bigpsltopsl
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname ucsc-bigpsltopsl
...
For e.g. docker, run::
    ...
    docker pull quay.io/biocontainers/ucsc-bigpsltopsl:<tag>

(see `ucsc-bigpsltopsl/tags`_ for valid values for ``<tag>``).
...
.. _`ucsc-bigpsltopsl/tags`: https://quay.io/repository/biocontainers/ucsc-bigpsltopsl?tab=tags


.. raw:: html

   <script>
      var package = "ucsc-bigpsltopsl";
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ucsc-bigpsltopsl"></div>
   <div style="width: 100%" id="platform_plot_ucsc-bigpsltopsl"></div>
   <div style="width: 100%" id="cdf_plot_ucsc-bigpsltopsl"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ucsc-bigpsltopsl
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-bigpsltopsl/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ucsc-bigpsltopsl', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ucsc-bigpsltopsl
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-bigpsltopsl/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ucsc-bigpsltopsl', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ucsc-bigpsltopsl
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-bigpsltopsl/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ucsc-bigpsltopsl', spec);
            } catch (err) {
