...
to install globally, run::
    ...
    pixi global install sspace_basic

to add into an existing workspace instead, run::

    pixi add sspace_basic
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install sspace_basic

Alternatively, to install into a new environment, run::

    conda create -n envname sspace_basic
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_sspace_basic"></div>
   <div style="width: 100%" id="platform_plot_sspace_basic"></div>
   <div style="width: 100%" id="cdf_plot_sspace_basic"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for sspace_basic
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/sspace_basic/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_sspace_basic', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for sspace_basic
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/sspace_basic/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_sspace_basic', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for sspace_basic
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/sspace_basic/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_sspace_basic', spec);
            } catch (err) {
