...
.. conda:recipe:: bioconductor-sigcheck
   ...
   :homepage: https://bioconductor.org/packages/3.22/bioc/html/SigCheck.html
...
.. conda:package:: bioconductor-sigcheck
   ...
   :versions:
      ...
      ``2.42.0-0``,  ``2.38.0-0``,  ``2.34.0-0``,  ``2.32.0-0``,  ``2.30.0-0``,  ``2.26.0-0``,  ``2.24.0-0``,  ``2.22.0-1``,  ``2.22.0-0``,  ``2.20.0-0``,  ``2.18.0-0``,  ``2.16.0-1``,  ``2.14.0-0``
...
to install globally, run::
    ...
    pixi global install bioconductor-sigcheck

to add into an existing workspace instead, run::

    pixi add bioconductor-sigcheck
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-sigcheck
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-sigcheck"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-sigcheck"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-sigcheck"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-sigcheck
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-sigcheck/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-sigcheck', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-sigcheck
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-sigcheck/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-sigcheck', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-sigcheck
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-sigcheck/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-sigcheck', spec);
            } catch (err) {
