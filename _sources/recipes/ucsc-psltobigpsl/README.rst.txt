...
.. conda:package:: ucsc-psltobigpsl
   ...
   :versions:
      ...
      ``482-0``,  ``469-1``,  ``469-0``,  ``377-4``,  ``377-3``,  ``377-2``,  ``377-1``,  ``377-0``,  ``366-0``,  ``357-2``,  ``357-1``,  ``357-0``,  ``332-0``,  ``324-0``
...
to install globally, run::
    ...
    pixi global install ucsc-psltobigpsl

to add into an existing workspace instead, run::

    pixi add ucsc-psltobigpsl
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname ucsc-psltobigpsl
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ucsc-psltobigpsl"></div>
   <div style="width: 100%" id="platform_plot_ucsc-psltobigpsl"></div>
   <div style="width: 100%" id="cdf_plot_ucsc-psltobigpsl"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ucsc-psltobigpsl
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-psltobigpsl/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ucsc-psltobigpsl', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ucsc-psltobigpsl
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-psltobigpsl/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ucsc-psltobigpsl', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ucsc-psltobigpsl
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-psltobigpsl/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ucsc-psltobigpsl', spec);
            } catch (err) {
