...
.. conda:package:: pycistarget
   ...
   :depends on ctxcore: ``>=0.2.0,<0.3.0``
   :depends on h5py: ``>=3.10.0,<4.0.0``
   :depends on ipython: ``>=8.0.0``
   :depends on numpy: ``<2.0.0``
   :depends on pandas: ``>=1.5.0,<2.0.0``
   :depends on pyranges: ``>=0.0.111,<0.1.0``
...
to install globally, run::
    ...
    pixi global install pycistarget

to add into an existing workspace instead, run::

    pixi add pycistarget
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install pycistarget

Alternatively, to install into a new environment, run::

    conda create -n envname pycistarget
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_pycistarget"></div>
   <div style="width: 100%" id="platform_plot_pycistarget"></div>
   <div style="width: 100%" id="cdf_plot_pycistarget"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for pycistarget
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/pycistarget/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_pycistarget', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for pycistarget
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/pycistarget/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_pycistarget', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for pycistarget
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/pycistarget/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_pycistarget', spec);
            } catch (err) {
