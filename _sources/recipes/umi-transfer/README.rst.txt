...
.. conda:package:: umi-transfer
   ...
   :versions:
      ...
      ``1.6.0-0``,  ``1.5.0-1``,  ``1.5.0-0``,  ``1.0.0-0``
...
to install globally, run::
    ...
    pixi global install umi-transfer

to add into an existing workspace instead, run::

    pixi add umi-transfer
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname umi-transfer
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["1.6.0","1.5.0","1.5.0","1.0.0"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_umi-transfer"></div>
   <div style="width: 100%" id="platform_plot_umi-transfer"></div>
   <div style="width: 100%" id="cdf_plot_umi-transfer"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for umi-transfer
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/umi-transfer/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_umi-transfer', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for umi-transfer
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/umi-transfer/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_umi-transfer', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for umi-transfer
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/umi-transfer/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_umi-transfer', spec);
            } catch (err) {
