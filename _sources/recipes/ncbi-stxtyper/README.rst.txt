...
.. conda:package:: ncbi-stxtyper
   ...
   :versions:
      ...
      ``1.0.45-0``,  ``1.0.42-0``,  ``1.0.40-0``,  ``1.0.31-1``,  ``1.0.31-0``,  ``1.0.27-0``,  ``1.0.25-0``,  ``1.0.24-0``
...
to install globally, run::
    ...
    pixi global install ncbi-stxtyper

to add into an existing workspace instead, run::

    pixi add ncbi-stxtyper
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install ncbi-stxtyper

Alternatively, to install into a new environment, run::

    conda create -n envname ncbi-stxtyper
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["1.0.45","1.0.42","1.0.40","1.0.31","1.0.31"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ncbi-stxtyper"></div>
   <div style="width: 100%" id="platform_plot_ncbi-stxtyper"></div>
   <div style="width: 100%" id="cdf_plot_ncbi-stxtyper"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ncbi-stxtyper
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ncbi-stxtyper/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ncbi-stxtyper', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ncbi-stxtyper
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ncbi-stxtyper/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ncbi-stxtyper', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ncbi-stxtyper
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ncbi-stxtyper/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ncbi-stxtyper', spec);
            } catch (err) {
