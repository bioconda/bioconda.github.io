...
.. conda:package:: scenicplus
   ...
   :depends on numpy: 
   :depends on pandas: 
   :depends on pycistarget: 
   :depends on python: ``>=3.8,<3.11.9``
...
to install globally, run::
    ...
    pixi global install scenicplus
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install scenicplus

Alternatively, to install into a new environment, run::

    conda create -n envname scenicplus
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_scenicplus"></div>
   <div style="width: 100%" id="platform_plot_scenicplus"></div>
   <div style="width: 100%" id="cdf_plot_scenicplus"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/scenicplus/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_scenicplus', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for scenicplus
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/scenicplus/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_scenicplus', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for scenicplus
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/scenicplus/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_scenicplus', spec);
            } catch (err) {
