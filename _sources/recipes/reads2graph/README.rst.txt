...
.. conda:package:: reads2graph
   ...
   :depends on boost: ``1.82.0``
   :depends on libgcc: ``>=13``
   :depends on libstdcxx: ``>=13``
   :depends on openmp: ``8.0.1``
   :depends on seqan3: ``3.3.0``
   :depends on seqan3: ``>=3.3.0,<4.0a0``
   :depends on sharg: ``1.1.1``
   :depends on sharg: ``>=1.1.1,<2.0a0``
...
to install globally, run::
    ...
    pixi global install reads2graph

to add into an existing workspace instead, run::

    pixi add reads2graph
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install reads2graph

Alternatively, to install into a new environment, run::

    conda create -n envname reads2graph
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_reads2graph"></div>
   <div style="width: 100%" id="platform_plot_reads2graph"></div>
   <div style="width: 100%" id="cdf_plot_reads2graph"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for reads2graph
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/reads2graph/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_reads2graph', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for reads2graph
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/reads2graph/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_reads2graph', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for reads2graph
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/reads2graph/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_reads2graph', spec);
            } catch (err) {
