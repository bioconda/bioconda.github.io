...
to install globally, run::
    ...
    pixi global install ucsc-maffrags

to add into an existing workspace instead, run::

    pixi add ucsc-maffrags
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname ucsc-maffrags
...
For e.g. docker, run::
    ...
    docker pull quay.io/biocontainers/ucsc-maffrags:<tag>
...
.. |downloads_ucsc-maffrags| image:: https://img.shields.io/conda/dn/bioconda/ucsc-maffrags.svg?style=flat
   :target: https://anaconda.org/bioconda/ucsc-maffrags
   :alt:   (downloads)
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ucsc-maffrags"></div>
   <div style="width: 100%" id="platform_plot_ucsc-maffrags"></div>
   <div style="width: 100%" id="cdf_plot_ucsc-maffrags"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ucsc-maffrags
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-maffrags/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ucsc-maffrags', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ucsc-maffrags
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-maffrags/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ucsc-maffrags', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ucsc-maffrags
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-maffrags/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ucsc-maffrags', spec);
            } catch (err) {
