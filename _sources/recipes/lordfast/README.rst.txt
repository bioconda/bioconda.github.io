...
.. conda:package:: lordfast
   ...
   :versions:
      ...
      ``0.0.10-6``,  ``0.0.10-5``,  ``0.0.10-4``,  ``0.0.10-3``,  ``0.0.10-2``,  ``0.0.10-1``,  ``0.0.10-0``,  ``0.0.9-0``
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname lordfast
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_lordfast"></div>
   <div style="width: 100%" id="platform_plot_lordfast"></div>
   <div style="width: 100%" id="cdf_plot_lordfast"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/lordfast/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_lordfast', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for lordfast
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/lordfast/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_lordfast', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for lordfast
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/lordfast/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_lordfast', spec);
            } catch (err) {
