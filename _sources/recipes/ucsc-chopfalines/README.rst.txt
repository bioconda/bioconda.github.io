...
to install globally, run::
    ...
    pixi global install ucsc-chopfalines

to add into an existing workspace instead, run::

    pixi add ucsc-chopfalines
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname ucsc-chopfalines
...
For e.g. docker, run::
    ...
    docker pull quay.io/biocontainers/ucsc-chopfalines:<tag>

(see `ucsc-chopfalines/tags`_ for valid values for ``<tag>``).
...
.. _`ucsc-chopfalines/tags`: https://quay.io/repository/biocontainers/ucsc-chopfalines?tab=tags


.. raw:: html

   <script>
      var package = "ucsc-chopfalines";
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ucsc-chopfalines"></div>
   <div style="width: 100%" id="platform_plot_ucsc-chopfalines"></div>
   <div style="width: 100%" id="cdf_plot_ucsc-chopfalines"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ucsc-chopfalines
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-chopfalines/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ucsc-chopfalines', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ucsc-chopfalines
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-chopfalines/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ucsc-chopfalines', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ucsc-chopfalines
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-chopfalines/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ucsc-chopfalines', spec);
            } catch (err) {
