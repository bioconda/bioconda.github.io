...
.. conda:package:: chronumental
   ...
   :versions:
      ...
      ``0.0.65-0``,  ``0.0.63-0``,  ``0.0.61-0``,  ``0.0.50-0``,  ``0.0.49-0``,  ``0.0.48-0``,  ``0.0.47-0``
...
to install globally, run::
    ...
    pixi global install chronumental

to add into an existing workspace instead, run::

    pixi add chronumental
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname chronumental
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["0.0.65","0.0.63","0.0.61","0.0.50","0.0.49"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_chronumental"></div>
   <div style="width: 100%" id="platform_plot_chronumental"></div>
   <div style="width: 100%" id="cdf_plot_chronumental"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for chronumental
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/chronumental/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_chronumental', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for chronumental
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/chronumental/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_chronumental', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for chronumental
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/chronumental/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_chronumental', spec);
            } catch (err) {
