...
.. conda:package:: tsebra
   ...
   :versions:
      ...
      ``1.1.2.5-0``,  ``1.1.2.4-0``,  ``1.1.2.3-0``,  ``1.1.2.2-0``,  ``1.1.2.1-0``,  ``1.1.2-0``,  ``1.1.1-0``

      

   
   :depends on numpy: 
   :depends on python: ``>=3.5.2``
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install tsebra

Alternatively, to install into a new environment, run::

    conda create -n envname tsebra
...
.. raw:: html
   ...
   <script>
      var package = "tsebra";
      var versions = ["1.1.2.5","1.1.2.4","1.1.2.3","1.1.2.2","1.1.2.1"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_tsebra"></div>
   <div style="width: 100%" id="platform_plot_tsebra"></div>
   <div style="width: 100%" id="cdf_plot_tsebra"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/tsebra/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_tsebra', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for tsebra
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/tsebra/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_tsebra', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for tsebra
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/tsebra/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_tsebra', spec);
            } catch (err) {
