...
to install globally, run::
    ...
    pixi global install ucsc-twobitmask

to add into an existing workspace instead, run::

    pixi add ucsc-twobitmask
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname ucsc-twobitmask
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ucsc-twobitmask"></div>
   <div style="width: 100%" id="platform_plot_ucsc-twobitmask"></div>
   <div style="width: 100%" id="cdf_plot_ucsc-twobitmask"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ucsc-twobitmask
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-twobitmask/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ucsc-twobitmask', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ucsc-twobitmask
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-twobitmask/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ucsc-twobitmask', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ucsc-twobitmask
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-twobitmask/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ucsc-twobitmask', spec);
            } catch (err) {
