...
.. conda:package:: mtsv
   ...
   :versions:
      ...
      ``1.0.6-5``,  ``1.0.6-2``,  ``1.0.6-1``,  ``1.0.6-0``,  ``1.0.5-0``,  ``1.0.4-0``,  ``1.0.3-0``,  ``1.0.2-0``

      

   
   :depends on biopython: 
   :depends on bracken: ``>=2.6.1``
   :depends on bwa: 
   :depends on click: 
   :depends on concoct: 
   :depends on ete3: 
   ...
   :depends on scipy: 
   :depends on six: 
   :depends on snakemake-minimal: ``>=4.1.0``
   :depends on wgfast: 
   :depends on zlib: 
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_mtsv"></div>
   <div style="width: 100%" id="platform_plot_mtsv"></div>
   <div style="width: 100%" id="cdf_plot_mtsv"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/mtsv/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_mtsv', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for mtsv
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/mtsv/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_mtsv', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for mtsv
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/mtsv/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_mtsv', spec);
            } catch (err) {
