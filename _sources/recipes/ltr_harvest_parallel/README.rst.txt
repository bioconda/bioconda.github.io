...
.. conda:package:: ltr_harvest_parallel
   ...
   :versions:
      ...
      ``1.3-0``,  ``1.2-2``,  ``1.2-1``,  ``1.2-0``,  ``1.1-1``,  ``1.1-0``
...
to install globally, run::
    ...
    pixi global install ltr_harvest_parallel

to add into an existing workspace instead, run::

    pixi add ltr_harvest_parallel
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install ltr_harvest_parallel

Alternatively, to install into a new environment, run::

    conda create -n envname ltr_harvest_parallel
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["1.3","1.2","1.2","1.2","1.1"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ltr_harvest_parallel"></div>
   <div style="width: 100%" id="platform_plot_ltr_harvest_parallel"></div>
   <div style="width: 100%" id="cdf_plot_ltr_harvest_parallel"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ltr_harvest_parallel
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ltr_harvest_parallel/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ltr_harvest_parallel', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ltr_harvest_parallel
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ltr_harvest_parallel/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ltr_harvest_parallel', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ltr_harvest_parallel
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ltr_harvest_parallel/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ltr_harvest_parallel', spec);
            } catch (err) {
