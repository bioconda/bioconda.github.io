...
to install globally, run::
    ...
    pixi global install ucsc-bedgraphpack

to add into an existing workspace instead, run::

    pixi add ucsc-bedgraphpack
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname ucsc-bedgraphpack
...
For e.g. docker, run::
    ...
    docker pull quay.io/biocontainers/ucsc-bedgraphpack:<tag>

(see `ucsc-bedgraphpack/tags`_ for valid values for ``<tag>``).
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ucsc-bedgraphpack"></div>
   <div style="width: 100%" id="platform_plot_ucsc-bedgraphpack"></div>
   <div style="width: 100%" id="cdf_plot_ucsc-bedgraphpack"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ucsc-bedgraphpack
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-bedgraphpack/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ucsc-bedgraphpack', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ucsc-bedgraphpack
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-bedgraphpack/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ucsc-bedgraphpack', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ucsc-bedgraphpack
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-bedgraphpack/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ucsc-bedgraphpack', spec);
            } catch (err) {
