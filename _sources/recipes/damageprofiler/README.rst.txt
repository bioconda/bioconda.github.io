.. _`damageprofiler`:

damageprofiler
==============

|downloads|

A Java based tool to determine damage patterns on ancient DNA as a replacement for mapDamage

============= ===========
Home          https://github.com/Integrative-Transcriptomics/DamageProfiler
Versions      0.3.11, 0.4.2, 0.4.3, 0.4.4
License       GPL-3.0
Recipe        https://github.com/bioconda/bioconda-recipes/tree/master/recipes/damageprofiler



============= ===========



Installation
------------

.. highlight: bash

With an activated Bioconda channel (see :ref:`set-up-channels`), install with::

   conda install damageprofiler

and update with::

   conda update damageprofiler



|docker|

A Docker container is available at https://quay.io/repository/biocontainers/damageprofiler.

Link to this page
-----------------

Render an |badge| badge with the following Markdown::

   [![install with bioconda](https://img.shields.io/badge/install%20with-bioconda-brightgreen.svg?style=flat-square)](http://bioconda.github.io/recipes/damageprofiler/README.html)

.. |badge| image:: https://img.shields.io/badge/install%20with-bioconda-brightgreen.svg?style=flat-square
           :target: http://bioconda.github.io/recipes/damageprofiler/README.html
.. |downloads| image:: https://anaconda.org/bioconda/damageprofiler/badges/downloads.svg
               :target: https://anaconda.org/bioconda/damageprofiler
.. |docker| image:: https://quay.io/repository/biocontainers/damageprofiler/status
                :target: https://quay.io/repository/biocontainers/damageprofiler

