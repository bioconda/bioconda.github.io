.. _`bioconductor-makecdfenv`:

bioconductor-makecdfenv
=======================

|downloads|

This package has two functions. One reads a Affymetrix chip description file \(CDF\) and creates a hash table environment containing the location\/probe set membership mapping. The other creates a package that automatically loads that environment.

============= ===========
Home          http://bioconductor.org/packages/3.7/bioc/html/makecdfenv.html
Versions      1.52.0, 1.54.0, 1.56.0
License       GPL (>= 2)
Recipe        https://github.com/bioconda/bioconda-recipes/tree/master/recipes/bioconductor-makecdfenv



Links         biotools: :biotools:`makecdfenv`, doi: :doi:`10.1186/1471-2105-13-56`

============= ===========



Installation
------------

.. highlight: bash

With an activated Bioconda channel (see :ref:`set-up-channels`), install with::

   conda install bioconductor-makecdfenv

and update with::

   conda update bioconductor-makecdfenv



|docker|

A Docker container is available at https://quay.io/repository/biocontainers/bioconductor-makecdfenv.

Link to this page
-----------------

Render an |badge| badge with the following Markdown::

   [![install with bioconda](https://img.shields.io/badge/install%20with-bioconda-brightgreen.svg?style=flat-square)](http://bioconda.github.io/recipes/bioconductor-makecdfenv/README.html)

.. |badge| image:: https://img.shields.io/badge/install%20with-bioconda-brightgreen.svg?style=flat-square
           :target: http://bioconda.github.io/recipes/bioconductor-makecdfenv/README.html
.. |downloads| image:: https://anaconda.org/bioconda/bioconductor-makecdfenv/badges/downloads.svg
               :target: https://anaconda.org/bioconda/bioconductor-makecdfenv
.. |docker| image:: https://quay.io/repository/biocontainers/bioconductor-makecdfenv/status
                :target: https://quay.io/repository/biocontainers/bioconductor-makecdfenv

