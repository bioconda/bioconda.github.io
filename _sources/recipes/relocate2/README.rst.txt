...
.. conda:package:: relocate2
   ...
   :depends on bedtools: ``2.26.0-0``
   :depends on blat: ``35``
   :depends on bowtie2: ``2.2.8``
   :depends on bwa: ``0.6.2-0``
   :depends on perl: ``>=5.10.0``
   :depends on pysam: ``0.9.0``
   :depends on python: ``2.7.12``
   :depends on samtools: ``1.3-0``
   :depends on seqtk: ``1.2-0``
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname relocate2
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_relocate2"></div>
   <div style="width: 100%" id="platform_plot_relocate2"></div>
   <div style="width: 100%" id="cdf_plot_relocate2"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/relocate2/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_relocate2', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for relocate2
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/relocate2/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_relocate2', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for relocate2
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/relocate2/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_relocate2', spec);
            } catch (err) {
