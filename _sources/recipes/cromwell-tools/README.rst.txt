...
.. conda:package:: cromwell-tools
   ...
   :depends on google-api-python-client: ``1.7.11.*``
   :depends on google-auth: ``>=1.6.1,<2``
   :depends on python: ``>=3.6``
   :depends on python-dateutil: ``2.8.0.*``
   :depends on requests: ``>=2.20.0,<3``
   :depends on setuptools_scm: ``>=3.1.0,<4``
...
to install globally, run::
    ...
    pixi global install cromwell-tools

to add into an existing workspace instead, run::

    pixi add cromwell-tools
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname cromwell-tools
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_cromwell-tools"></div>
   <div style="width: 100%" id="platform_plot_cromwell-tools"></div>
   <div style="width: 100%" id="cdf_plot_cromwell-tools"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for cromwell-tools
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/cromwell-tools/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_cromwell-tools', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for cromwell-tools
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/cromwell-tools/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_cromwell-tools', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for cromwell-tools
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/cromwell-tools/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_cromwell-tools', spec);
            } catch (err) {
