...
.. conda:package:: blobtoolkit
   ...
   :versions:
      ...
      ``4.5.5-0``,  ``4.5.3-0``,  ``4.5.2-0``,  ``4.5.1-1``,  ``4.5.1-0``,  ``4.5.0-0``,  ``4.4.6-0``

      

   
   :depends on blobtk: ``>=0.7.1``
   :depends on chromedriver-autoinstaller: ``>=0.6.2``
   :depends on docopt: ``>=0.6.2``
   :depends on firefox: 
   :depends on geckodriver: 
   :depends on psutil: ``5.9.4``
   :depends on python: ``>=3.9,<3.14``
   :depends on python-fastjsonschema: ``2.21.1``
   :depends on pyvirtualdisplay: ``3.0``
   :depends on pyyaml: 
   :depends on selenium: ``>=4.10.0``
   :depends on tolkein: ``>=0.5.0``
   :depends on tqdm: ``4.64.1``
   :depends on ujson: ``>=5.7.0``
...
to install globally, run::
    ...
    pixi global install blobtoolkit

to add into an existing workspace instead, run::

    pixi add blobtoolkit
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install blobtoolkit

Alternatively, to install into a new environment, run::

    conda create -n envname blobtoolkit
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["4.5.5","4.5.3","4.5.2","4.5.1","4.5.1"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_blobtoolkit"></div>
   <div style="width: 100%" id="platform_plot_blobtoolkit"></div>
   <div style="width: 100%" id="cdf_plot_blobtoolkit"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for blobtoolkit
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/blobtoolkit/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_blobtoolkit', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for blobtoolkit
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/blobtoolkit/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_blobtoolkit', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for blobtoolkit
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/blobtoolkit/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_blobtoolkit', spec);
            } catch (err) {
