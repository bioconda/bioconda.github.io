...
.. conda:package:: strainseeker
   ...
   :versions:
      ...
      ``1.5.1-5``,  ``1.5.1-4``,  ``1.5.1-3``,  ``1.5.1-2``,  ``1.5.1-1``,  ``1.5.1-0``
...
to install globally, run::
    ...
    pixi global install strainseeker

to add into an existing workspace instead, run::

    pixi add strainseeker
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname strainseeker
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_strainseeker"></div>
   <div style="width: 100%" id="platform_plot_strainseeker"></div>
   <div style="width: 100%" id="cdf_plot_strainseeker"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for strainseeker
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/strainseeker/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_strainseeker', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for strainseeker
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/strainseeker/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_strainseeker', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for strainseeker
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/strainseeker/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_strainseeker', spec);
            } catch (err) {
