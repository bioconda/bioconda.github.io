...
.. conda:recipe:: bioconductor-abarray
   ...
   :homepage: https://bioconductor.org/packages/3.22/bioc/html/ABarray.html
   :license: GPL
...
.. conda:package:: bioconductor-abarray
   ...
   :versions:
      ...
      ``1.78.0-0``,  ``1.74.0-0``,  ``1.70.0-0``,  ``1.68.0-0``,  ``1.66.0-0``,  ``1.62.0-0``,  ``1.60.0-0``,  ``1.58.0-1``,  ``1.58.0-0``,  ``1.56.0-0``,  ``1.54.0-0``,  ``1.52.0-1``,  ``1.50.0-0``
...
to install globally, run::
    ...
    pixi global install bioconductor-abarray

to add into an existing workspace instead, run::

    pixi add bioconductor-abarray
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-abarray
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-abarray"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-abarray"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-abarray"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-abarray
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-abarray/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-abarray', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-abarray
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-abarray/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-abarray', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-abarray
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-abarray/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-abarray', spec);
            } catch (err) {
