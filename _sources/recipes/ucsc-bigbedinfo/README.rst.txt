...
to install globally, run::
    ...
    pixi global install ucsc-bigbedinfo

to add into an existing workspace instead, run::

    pixi add ucsc-bigbedinfo
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname ucsc-bigbedinfo
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_ucsc-bigbedinfo"></div>
   <div style="width: 100%" id="platform_plot_ucsc-bigbedinfo"></div>
   <div style="width: 100%" id="cdf_plot_ucsc-bigbedinfo"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for ucsc-bigbedinfo
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-bigbedinfo/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_ucsc-bigbedinfo', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for ucsc-bigbedinfo
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-bigbedinfo/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_ucsc-bigbedinfo', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for ucsc-bigbedinfo
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/ucsc-bigbedinfo/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_ucsc-bigbedinfo', spec);
            } catch (err) {
