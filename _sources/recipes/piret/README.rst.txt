...
.. conda:package:: piret
   ...
   :depends on bioconductor-deseq2: ``>=1.20.0``
   :depends on bioconductor-edger: ``>=3.22.5``
   :depends on biopython: ``>=1.7.0``
   :depends on hisat2: ``>=2.0.5``
   :depends on luigi: ``>=2.7.9``
   :depends on pandas: ``>=0.23.4``
   :depends on plumbum: ``>=1.6.0``
   :depends on python: 
   :depends on r-argparse: ``>=2.0.0``
   :depends on r-base: ``>=4.4,<4.5.0a0``
   :depends on samtools: ``>=1.3.1``
   :depends on stringtie: ``>=1.3.3``
   :depends on subread: ``>=1.5.0``
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_piret"></div>
   <div style="width: 100%" id="platform_plot_piret"></div>
   <div style="width: 100%" id="cdf_plot_piret"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/piret/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_piret', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for piret
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/piret/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_piret', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for piret
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/piret/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_piret', spec);
            } catch (err) {
