...
.. conda:package:: variant_tools
   ...
   :depends on blosc: ``>=1.21.6,<2.0a0``
   :depends on boost-cpp: ``<1.73.0``
...
to install globally, run::
    ...
    pixi global install variant_tools

to add into an existing workspace instead, run::

    pixi add variant_tools
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname variant_tools
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_variant_tools"></div>
   <div style="width: 100%" id="platform_plot_variant_tools"></div>
   <div style="width: 100%" id="cdf_plot_variant_tools"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for variant_tools
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/variant_tools/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_variant_tools', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for variant_tools
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/variant_tools/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_variant_tools', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for variant_tools
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/variant_tools/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_variant_tools', spec);
            } catch (err) {
