...
.. conda:recipe:: bioconductor-flowploidy
   ...
   :homepage: https://bioconductor.org/packages/3.22/bioc/html/flowPloidy.html
   :license: GPL-3
...
.. conda:package:: bioconductor-flowploidy
   ...
   :versions:
      ...
      ``1.36.0-0``,  ``1.32.0-0``,  ``1.28.0-0``,  ``1.26.0-0``,  ``1.24.0-0``,  ``1.20.0-0``,  ``1.18.0-0``,  ``1.16.0-1``,  ``1.16.0-0``,  ``1.14.0-0``,  ``1.12.0-0``,  ``1.10.0-1``,  ``1.8.0-0``,  ``1.6.0-0``,  ``1.4.1-0``
...
to install globally, run::
    ...
    pixi global install bioconductor-flowploidy

to add into an existing workspace instead, run::

    pixi add bioconductor-flowploidy
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-flowploidy
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-flowploidy"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-flowploidy"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-flowploidy"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-flowploidy
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-flowploidy/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-flowploidy', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-flowploidy
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-flowploidy/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-flowploidy', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-flowploidy
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-flowploidy/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-flowploidy', spec);
            } catch (err) {
