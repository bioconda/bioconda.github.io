...
.. conda:package:: debarcer
   ...
   :depends on matplotlib-base: ``>=3.1,<=3.3``
   :depends on mistune: ``>=0.8,<2``
   :depends on networkx: ``2.4.*``
   :depends on numpy: ``>=1.14``
   :depends on pandas: ``>=0.22``
   :depends on pygal: ``>=2.4``
   :depends on pysam: ``>=0.14``
   :depends on python: ``>=3.6``
   :depends on pyyaml: ``>=5.1``
   :depends on scipy: ``>=1.1,<=1.2``
   :depends on seaborn: ``>=0.9,<=0.10``
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname debarcer
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_debarcer"></div>
   <div style="width: 100%" id="platform_plot_debarcer"></div>
   <div style="width: 100%" id="cdf_plot_debarcer"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/debarcer/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_debarcer', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for debarcer
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/debarcer/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_debarcer', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for debarcer
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/debarcer/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_debarcer', spec);
            } catch (err) {
