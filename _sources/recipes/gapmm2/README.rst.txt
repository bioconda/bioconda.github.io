...
.. conda:package:: gapmm2
   ...
   :versions:
      ...
      ``26.5.22-0``,  ``25.8.12-0``,  ``25.4.13-0``,  ``25.4.5-0``,  ``23.11.3-0``,  ``0.2.0-0``
...
.. raw:: html
   ...
   <script>
      var package = "gapmm2";
      var versions = ["26.5.22","25.8.12","25.4.13","25.4.5","23.11.3"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_gapmm2"></div>
   <div style="width: 100%" id="platform_plot_gapmm2"></div>
   <div style="width: 100%" id="cdf_plot_gapmm2"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/gapmm2/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_gapmm2', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for gapmm2
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/gapmm2/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_gapmm2', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for gapmm2
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/gapmm2/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_gapmm2', spec);
            } catch (err) {
