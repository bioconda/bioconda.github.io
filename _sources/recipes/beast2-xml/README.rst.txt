...
.. conda:package:: beast2-xml
   ...
   :versions:
      ...
      ``1.6.2-0``,  ``1.6.1-0``,  ``1.5.2-0``,  ``1.5.1-0``,  ``1.5.0-0``
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["1.6.2","1.6.1","1.5.2","1.5.1","1.5.0"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_beast2-xml"></div>
   <div style="width: 100%" id="platform_plot_beast2-xml"></div>
   <div style="width: 100%" id="cdf_plot_beast2-xml"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/beast2-xml/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_beast2-xml', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for beast2-xml
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/beast2-xml/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_beast2-xml', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for beast2-xml
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/beast2-xml/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_beast2-xml', spec);
            } catch (err) {
