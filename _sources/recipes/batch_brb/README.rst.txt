...
.. conda:package:: batch_brb
   ...
   :depends on perl-fast: 
   :depends on python: ``>=3.8.6``
   :depends on sed: ``>=4.8``
   :depends on seqkit: ``>=0.13.2``
   :depends on sqlite: ``>=3.33.0``
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install batch_brb

Alternatively, to install into a new environment, run::

    conda create -n envname batch_brb
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_batch_brb"></div>
   <div style="width: 100%" id="platform_plot_batch_brb"></div>
   <div style="width: 100%" id="cdf_plot_batch_brb"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/batch_brb/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_batch_brb', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for batch_brb
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/batch_brb/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_batch_brb', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for batch_brb
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/batch_brb/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_batch_brb', spec);
            } catch (err) {
