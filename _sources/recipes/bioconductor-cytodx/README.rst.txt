...
.. conda:recipe:: bioconductor-cytodx
   ...
   :homepage: https://bioconductor.org/packages/3.22/bioc/html/CytoDx.html
   :license: GPL-2
...
.. conda:package:: bioconductor-cytodx
   ...
   :versions:
      ...
      ``1.30.0-0``,  ``1.26.0-0``,  ``1.22.0-0``,  ``1.20.0-0``,  ``1.18.0-0``,  ``1.14.0-0``,  ``1.12.0-0``,  ``1.10.0-1``,  ``1.10.0-0``,  ``1.8.0-0``,  ``1.6.0-0``,  ``1.4.0-1``,  ``1.2.1-1``,  ``1.2.1-0``
...
to install globally, run::
    ...
    pixi global install bioconductor-cytodx

to add into an existing workspace instead, run::

    pixi add bioconductor-cytodx
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-cytodx
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-cytodx"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-cytodx"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-cytodx"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-cytodx
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-cytodx/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-cytodx', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-cytodx
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-cytodx/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-cytodx', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-cytodx
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-cytodx/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-cytodx', spec);
            } catch (err) {
