...
to install globally, run::
    ...
    pixi global install bugseq-porechop

to add into an existing workspace instead, run::

    pixi add bugseq-porechop
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bugseq-porechop
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bugseq-porechop"></div>
   <div style="width: 100%" id="platform_plot_bugseq-porechop"></div>
   <div style="width: 100%" id="cdf_plot_bugseq-porechop"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bugseq-porechop
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bugseq-porechop/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bugseq-porechop', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bugseq-porechop
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bugseq-porechop/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bugseq-porechop', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bugseq-porechop
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bugseq-porechop/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bugseq-porechop', spec);
            } catch (err) {
