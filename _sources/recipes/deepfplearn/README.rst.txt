...
.. conda:package:: deepfplearn
   ...
   :depends on chemprop: ``1.7.1.*``
   :depends on jsonpickle: ``2.1.*``
   :depends on keras: ``2.10``
   :depends on matplotlib-base: ``3.5.1.*``
   :depends on numpy: ``1.22``
   :depends on pandas: ``1.4``
   :depends on protobuf: ``>3.19``
   :depends on python: 
   :depends on rdkit: ``2022.03.*``
   :depends on scikit-learn: ``1.0.*``
   :depends on seaborn: ``0.12.2.*``
   :depends on tensorflow-base: 
   :depends on umap-learn: ``0.5.*``
   :depends on wandb: ``0.12.20.*``
...
to install globally, run::
    ...
    pixi global install deepfplearn

to add into an existing workspace instead, run::

    pixi add deepfplearn
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install deepfplearn

Alternatively, to install into a new environment, run::

    conda create -n envname deepfplearn
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_deepfplearn"></div>
   <div style="width: 100%" id="platform_plot_deepfplearn"></div>
   <div style="width: 100%" id="cdf_plot_deepfplearn"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for deepfplearn
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/deepfplearn/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_deepfplearn', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for deepfplearn
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/deepfplearn/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_deepfplearn', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for deepfplearn
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/deepfplearn/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_deepfplearn', spec);
            } catch (err) {
