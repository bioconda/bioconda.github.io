...
.. conda:recipe:: bioconductor-netsmooth
   ...
   :homepage: https://bioconductor.org/packages/3.22/bioc/html/netSmooth.html
   :license: GPL-3
...
.. conda:package:: bioconductor-netsmooth
   ...
   :versions:
      ...
      ``1.30.0-0``,  ``1.26.0-0``,  ``1.22.0-0``,  ``1.20.0-0``,  ``1.18.0-0``,  ``1.14.0-0``,  ``1.12.0-0``,  ``1.10.0-1``,  ``1.10.0-0``,  ``1.8.0-0``,  ``1.6.0-0``,  ``1.4.0-1``,  ``1.2.0-1``,  ``1.2.0-0``
   ...
   :depends on bioconductor-clusterexperiment: ``>=2.30.0,<2.31.0``
...
to install globally, run::
    ...
    pixi global install bioconductor-netsmooth

to add into an existing workspace instead, run::

    pixi add bioconductor-netsmooth
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-netsmooth
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-netsmooth"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-netsmooth"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-netsmooth"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-netsmooth
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-netsmooth/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-netsmooth', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-netsmooth
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-netsmooth/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-netsmooth', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-netsmooth
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-netsmooth/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-netsmooth', spec);
            } catch (err) {
