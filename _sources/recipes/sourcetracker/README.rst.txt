...
.. conda:package:: sourcetracker
   ...
   :depends on biom-format: ``>=2.1.5,<2.2.0``
   :depends on click: 
   :depends on h5py: 
...
to install globally, run::
    ...
    pixi global install sourcetracker

to add into an existing workspace instead, run::

    pixi add sourcetracker
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname sourcetracker
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_sourcetracker"></div>
   <div style="width: 100%" id="platform_plot_sourcetracker"></div>
   <div style="width: 100%" id="cdf_plot_sourcetracker"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for sourcetracker
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/sourcetracker/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_sourcetracker', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for sourcetracker
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/sourcetracker/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_sourcetracker', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for sourcetracker
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/sourcetracker/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_sourcetracker', spec);
            } catch (err) {
