.. _`bioconductor-hivcdnavantwout03`:

bioconductor-hivcdnavantwout03
==============================

|downloads|

The expression levels of approximately 4600 cellular RNA transcripts were assessed in CD4\+ T cell lines at different times after infection with HIV\-1BRU using DNA microarrays. This data corresponds to the first block of a 12 block array image \(001030\_08\_1.GEL\) in the first data set \(2000095918 A\) in the first experiment \(CEM LAI vs HI\-LAI 24hr\). There are two data sets\, which are part of a dye\-swap experiment with replicates\, representing the Cy3 \(green\) absorption intensities for channel 1 \(hiv1raw\) and the Cy5 \(red\) absorption intensities for channel 2 \(hiv2raw\).

============= ===========
Home          https://bioconductor.org/packages/3.8/data/experiment/html/HIVcDNAvantWout03.html
Versions      1.22.0
License       GPL (>= 2)
Recipe        https://github.com/bioconda/bioconda-recipes/tree/master/recipes//bioconductor-hivcdnavantwout03/meta.yaml



============= ===========



Installation
------------

.. highlight: bash

With an activated Bioconda channel (see :ref:`set-up-channels`), install with::

   conda install bioconductor-hivcdnavantwout03

and update with::

   conda update bioconductor-hivcdnavantwout03



|docker|

A Docker container is available at https://quay.io/repository/biocontainers/bioconductor-hivcdnavantwout03.

Link to this page
-----------------

Render an |badge| badge with the following Markdown::

   [![install with bioconda](https://img.shields.io/badge/install%20with-bioconda-brightgreen.svg?style=flat-square)](http://bioconda.github.io/recipes/bioconductor-hivcdnavantwout03/README.html)

.. |badge| image:: https://img.shields.io/badge/install%20with-bioconda-brightgreen.svg?style=flat-square
           :target: http://bioconda.github.io/recipes/bioconductor-hivcdnavantwout03/README.html
.. |downloads| image:: https://anaconda.org/bioconda/bioconductor-hivcdnavantwout03/badges/downloads.svg
               :target: https://anaconda.org/bioconda/bioconductor-hivcdnavantwout03
.. |docker| image:: https://quay.io/repository/biocontainers/bioconductor-hivcdnavantwout03/status
                :target: https://quay.io/repository/biocontainers/bioconductor-hivcdnavantwout03

