...
.. conda:package:: quantpi
   ...
   :depends on bowtie2: 
   :depends on bracken: ``>=2.7``
   :depends on bwa: 
   :depends on coverm: ``>=0.6.1``
   :depends on fastp: 
   :depends on fastqc: 
   :depends on humann: 
   :depends on kmcp: ``>=0.8.2``
   :depends on kraken2: 
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname quantpi
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_quantpi"></div>
   <div style="width: 100%" id="platform_plot_quantpi"></div>
   <div style="width: 100%" id="cdf_plot_quantpi"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/quantpi/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_quantpi', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for quantpi
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/quantpi/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_quantpi', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for quantpi
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/quantpi/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_quantpi', spec);
            } catch (err) {
