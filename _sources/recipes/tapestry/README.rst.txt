...
.. conda:package:: tapestry
   ...
   :versions:
      ...
      ``1.0.1-0``,  ``1.0.0-0``,  ``0.9.3-1``,  ``0.9.3-0``,  ``0.9.2-0``,  ``0.9.1-0``
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname tapestry
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["1.0.1","1.0.0","0.9.3","0.9.3","0.9.2"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_tapestry"></div>
   <div style="width: 100%" id="platform_plot_tapestry"></div>
   <div style="width: 100%" id="cdf_plot_tapestry"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/tapestry/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_tapestry', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for tapestry
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/tapestry/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_tapestry', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for tapestry
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/tapestry/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_tapestry', spec);
            } catch (err) {
