...
to install globally, run::
    ...
    pixi global install mgf-formatter

to add into an existing workspace instead, run::

    pixi add mgf-formatter
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname mgf-formatter
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_mgf-formatter"></div>
   <div style="width: 100%" id="platform_plot_mgf-formatter"></div>
   <div style="width: 100%" id="cdf_plot_mgf-formatter"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for mgf-formatter
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/mgf-formatter/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_mgf-formatter', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for mgf-formatter
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/mgf-formatter/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_mgf-formatter', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for mgf-formatter
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/mgf-formatter/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_mgf-formatter', spec);
            } catch (err) {
