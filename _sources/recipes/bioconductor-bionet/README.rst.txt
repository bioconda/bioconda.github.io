...
.. conda:recipe:: bioconductor-bionet
   ...
   :homepage: https://bioconductor.org/packages/3.22/bioc/html/BioNet.html
   :license: GPL (>= 2)
...
.. conda:package:: bioconductor-bionet
   ...
   :versions:
      ...
      ``1.70.0-0``,  ``1.66.0-0``,  ``1.62.0-0``,  ``1.60.0-0``,  ``1.58.0-0``,  ``1.54.0-0``,  ``1.52.0-0``,  ``1.50.0-1``,  ``1.50.0-0``,  ``1.47.0-0``,  ``1.46.0-0``,  ``1.44.0-1``,  ``1.42.0-0``,  ``1.40.0-0``,  ``1.38.0-0``,  ``1.36.0-0``,  ``1.34.0-0``
...
to install globally, run::
    ...
    pixi global install bioconductor-bionet

to add into an existing workspace instead, run::

    pixi add bioconductor-bionet
...
Alternatively, to install into a new environment, run::
    ...
    conda create -n envname bioconductor-bionet
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_bioconductor-bionet"></div>
   <div style="width: 100%" id="platform_plot_bioconductor-bionet"></div>
   <div style="width: 100%" id="cdf_plot_bioconductor-bionet"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for bioconductor-bionet
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-bionet/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_bioconductor-bionet', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for bioconductor-bionet
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-bionet/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_bioconductor-bionet', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for bioconductor-bionet
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/bioconductor-bionet/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_bioconductor-bionet', spec);
            } catch (err) {
