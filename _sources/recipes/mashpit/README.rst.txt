...
.. conda:package:: mashpit
   ...
   :depends on dask-core: ``>=2024.1``
   :depends on flask: ``>=3.0.3``
   :depends on ncbi-datasets-cli: ``>=16.10``
   :depends on ncbi-datasets-pylib: ``>=16.6.0``
   :depends on numpy: ``<2.0.0``
   :depends on pandas: ``>=2.0.3``
   :depends on phytreeviz: 
   :depends on psutil: 
   :depends on python: ``>=3.9,<3.10``
   :depends on requests: 
   :depends on scikit-bio: 
   :depends on scipy: ``>=1.0``
   :depends on screed: ``>=1.0.5``
   :depends on sourmash: ``>=4.6.1``
   :depends on tqdm: 
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install mashpit

Alternatively, to install into a new environment, run::

    conda create -n envname mashpit
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_mashpit"></div>
   <div style="width: 100%" id="platform_plot_mashpit"></div>
   <div style="width: 100%" id="cdf_plot_mashpit"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/mashpit/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_mashpit', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for mashpit
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/mashpit/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_mashpit', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for mashpit
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/mashpit/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_mashpit', spec);
            } catch (err) {
