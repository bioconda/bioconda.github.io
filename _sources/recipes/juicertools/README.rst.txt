...
to install globally, run::
    ...
    pixi global install juicertools

to add into an existing workspace instead, run::

    pixi add juicertools
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install juicertools

Alternatively, to install into a new environment, run::

    conda create -n envname juicertools
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_juicertools"></div>
   <div style="width: 100%" id="platform_plot_juicertools"></div>
   <div style="width: 100%" id="cdf_plot_juicertools"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for juicertools
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/juicertools/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_juicertools', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for juicertools
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/juicertools/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_juicertools', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for juicertools
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/juicertools/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_juicertools', spec);
            } catch (err) {
