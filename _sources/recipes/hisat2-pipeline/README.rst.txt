...
.. conda:package:: hisat2-pipeline
   ...
   :versions:
      ...
      ``1.1.1-0``,  ``1.1.0-0``,  ``1.0.8-0``,  ``1.0.7-0``,  ``1.0.6-0``
...
to install globally, run::
    ...
    pixi global install hisat2-pipeline

to add into an existing workspace instead, run::

    pixi add hisat2-pipeline
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install hisat2-pipeline

Alternatively, to install into a new environment, run::

    conda create -n envname hisat2-pipeline
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["1.1.1","1.1.0","1.0.8","1.0.7","1.0.6"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_hisat2-pipeline"></div>
   <div style="width: 100%" id="platform_plot_hisat2-pipeline"></div>
   <div style="width: 100%" id="cdf_plot_hisat2-pipeline"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for hisat2-pipeline
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/hisat2-pipeline/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_hisat2-pipeline', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for hisat2-pipeline
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/hisat2-pipeline/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_hisat2-pipeline', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for hisat2-pipeline
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/hisat2-pipeline/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_hisat2-pipeline', spec);
            } catch (err) {
