...
.. conda:package:: konezumiaid
   ...
   :versions:
      ...
      ``0.3.6.1-0``,  ``0.3.5-0``,  ``0.3.1-0``,  ``0.3.0-0``,  ``0.2.2-0``
   ...
   :depends on primer3-py: ``>=2.0.1,<3.0.0``
...
to install globally, run::
    ...
    pixi global install konezumiaid

to add into an existing workspace instead, run::

    pixi add konezumiaid
...
With conda_ installed and the Bioconda channel set up (see :ref:`bioconda_setup`), to install into an existing and activated environment, run::
    ...
    conda install konezumiaid

Alternatively, to install into a new environment, run::

    conda create -n envname konezumiaid
...
.. raw:: html
   ...
   <script>
      ...
      var versions = ["0.3.6.1","0.3.5","0.3.1","0.3.0","0.2.2"];
   </script>
...
.. raw:: html
   ...
   <div style="width: 100%" id="download_plot_konezumiaid"></div>
   <div style="width: 100%" id="platform_plot_konezumiaid"></div>
   <div style="width: 100%" id="cdf_plot_konezumiaid"></div>
   ...
   <script>
      window.onload = async function() {
            ...
            // Build cdf plot for konezumiaid
            try {
               ...
               const point_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/konezumiaid/cdf.json`)
               ...
               vegaEmbed('#cdf_plot_konezumiaid', cdf_spec);
            } catch (err) {
               console.error("An error occurred while building CDF plot: ", err)
            }
    
            // Build download plot for konezumiaid
            try {
               ...
               const version_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/konezumiaid/versions.json`)
               if (!version_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${version_data_resp.status}.`);
               }
               const plot_data = await version_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#download_plot_konezumiaid', spec);
            } catch (err) {
               console.error("An error occurred while building downloads plot: ", err)
            }
   
            // Build platform download plot for konezumiaid
            try {
               ...
               const spec = await spec_resp.json();
               const platform_data_resp = await fetch(`https://raw.githubusercontent.com/bioconda/bioconda-plots/main/plots/konezumiaid/platforms.json`)
               if (!platform_data_resp.ok) {
                   throw new Error(`Fetching failed with HTTP code ${platform_data_resp.status}.`);
               }
               const plot_data = await platform_data_resp.json();
               spec.data.values = plot_data;
               vegaEmbed('#platform_plot_konezumiaid', spec);
            } catch (err) {
